...
- **Barcodes assigned at creation** (migration `20260604_0200`): previously `get_items` lazily generated a missing barcode and **committed it during a GET**, making a read path issue writes (and interacting badly with the stale-while-revalidate cache). Barcodes are now assigned at item-creation time through a single canonical generator (`INV-` + 8 uppercase hex characters, verified unique within the organization); item creation, variant generation, and label backfill all share it. The migration backfills legacy rows missing one with the same `INV-XXXXXXXX` shape. The leftover on-read generation in `get_item_by_id` was also removed
