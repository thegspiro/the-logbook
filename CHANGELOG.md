...
- **Barcodes assigned at creation** (migration `20260604_0200`): previously `get_items` lazily generated a missing barcode and **committed it during a GET**, making a read path issue writes (and interacting badly with the stale-while-revalidate cache). Barcodes are now assigned at item-creation time using **one canonical scheme everywhere** — `INV-` + the first 8 uppercase hex characters of the item's own UUID, verified unique within the organization (widened to 12 hex on the rare collision). Item creation, variant generation, the label backfill, and the one-time migration `20260604_0200` all follow this identical derivation, so every barcode in the table — historical-backfilled or newly-created — is reproducible from its row id. The leftover on-read generation in `get_item_by_id` was also removed
