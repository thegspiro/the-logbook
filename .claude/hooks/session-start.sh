...
(
  ...
  if ! python3 -m alembic upgrade head >/tmp/session-alembic.log 2>&1; then
    ...
    log "       Drop only — let this hook recreate it. A hand-rolled CREATE"
    log "       DATABASE inherits the server default (utf8mb4_general_ci here),"
    log "       and 20260120_0013's locations FK then fails with errno 150"
    log "       against the utf8mb4_unicode_ci the migrations declare — which"
    log "       reads as a broken chain and is not one."
