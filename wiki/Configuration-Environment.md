...
## Check the configuration before you restart _(2026-08-18)_
...
| Exit code | Meaning                                    |
| --------- | ------------------------------------------ |
| `0`       | The configuration starts                   |
| `1`       | It does not; the blocking items are listed |
| `2`       | A value is malformed                       |
...
- **The same `-f` files the deployment uses.** Compose merges values from every
  file given, so a bare `docker compose run` evaluates only the base development
  configuration: different `SECURITY_ENFORCE_HTTPS`, different `ENABLE_DOCS`,
  and an answer about a configuration nobody runs.

**`python -m app.preflight --compose PATH`** goes further and names the settings
a compose file **drops** — a value present in `.env` that never reaches the
container because nothing passes it through. Previously those silently became
...
| `SECURITY_REQUIRE_TLS`                | Promote **absent** `DB_SSL`/`REDIS_SSL` in production/staging from a boot **warning** to a **CRITICAL** finding, which the application refuses to start on. Without it, PHI, sessions and cached queries can cross the network in cleartext and nothing blocks the deployment _(2026-08-12)_                                                                                                                                                                                                                          | **`true`**            |

> **`SECURITY_REQUIRE_TLS` fails closed by default** _(2026-08-12)_. Set it to
> `false` only when a trusted private network, service mesh, or sidecar provides
> equivalent transport protection. That explicit override records the deployment
> owner's risk decision instead of silently permitting cleartext transport.
>
...
> the widget script and iframe, and the symptom is _"the challenge never
> appears"_ rather than anything naming the CSP. With CAPTCHA off the policy is
