...
| Meetings   | `list_meetings`, `list_open_action_items`, `list_minutes`, `get_minutes`, `get_minutes_text` — **approved, non-executive minutes only**; long text (the agenda, reports and business, a dynamic section, or a motion's discussion notes by motion id) is read in 20,000-character pieces |
...
| **Share finance totals**           | `list_fiscal_years`, `get_budget_summary`, `list_budgets`, and the finance sections inside published minutes (the treasurer's report, financial review, trust fund report, audit report, and any section whose name mentions money)                                       |
