...
- **External Integrations** — Connect external training providers (Vector Solutions, Target Solutions, Lexipol, iAmResponding, Custom API) with category and user mapping. *(2026-04-11)* Vector Solutions integration now includes upfront category catalog fetch, credit hours preservation, and improved type mapping with auto-sync. *(2026-07-16)* Imported courses now also feed **pipeline** progress: on import, a course advances any active-enrollment requirement it matches by category (HOURS accrues hours, COURSES accrues one completion), so synced coursework moves recruit-school/certification progress, not just the compliance matrix
