...
| **How calls are recorded** (`call_tracking.mode`) | `detailed` | Three modes. `detailed` logs individual calls on the shift panel and counts volume from those records; `count_only` swaps the finalize checklist for the three-step wizard and counts calls the officer reports; `off` hides the call log and asks nothing. Calls already recorded stay visible and removable in every mode _(2026-08-19, made three-way 2026-09-12)_ |
