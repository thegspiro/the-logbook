...
administrator turns it back on. Five upgrade steps **take permissions away**
...
- **`/testing` has disappeared.** The Testing Checklist is now a module and it
  ...
  back. **Nothing you recorded is lost** — every mark and note is still there
  and reappears the moment you switch it on.

- **Five upgrade steps revoke permissions from seeded positions**, and they are
  not granted back automatically:
...
- **Every administrative member has had their operational rank cleared.** An
  ...
  never meant to have. This does not reverse — if an administrative member
  genuinely holds an operational rank, set it again after the upgrade.
...
- **Facility files were readable by the whole department** through the
  ...
  pointed at was not. Fixed, and existing facility folders are corrected for
  you on upgrade. A documents administrator no longer sees facility files by
  virtue of that role alone.
...
- **Your profile now shows two facts where it showed one.** "Membership type"
  became **class** (operational / administrative / social) and **status**
  (prospective → probationary → regular → life → retired). A life member who
  still rides, or a probationary treasurer, is now expressible.
...
| **Inventory**                                   | Item detail; return-request review; transfer; receiving                           | Reorder-receiving, return-receipt and request-duration migrations                                                                                                                                 | "Checkout batch" is now **Item Distribution**, and the module's vocabulary is fixed in one place — Assignment, Temporary loan, Issuance, Return, Check-in, Transfer, Distribution. **API values are unchanged**; only the labels moved. **Stock received through the reorder workflow now actually becomes issuable** (it previously created a purchase record and never updated on-hand). An item recorded as poor, damaged or out of service **can no longer be returned to service** until a later inspection records a safe condition. The "Transfer is immediate" checkbox is gone — transfers always were                                                                                                                                         |
...
**Four migrations do not reverse**, and in three of the four a downgrade that
...
- **Re-check your compliance percentages.** The configuration fix can move
  ...
  move them a lot.
- **Set the operational rank again** on any administrative member who genuinely
  holds one.
