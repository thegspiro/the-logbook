...
39. [Mobile Member ID Scanner](#mobile-member-id-scanner-2026-03-02)
40. [API Service Split](#api-service-split-2026-03-02)
41. [exactOptionalPropertyTypes](#exactoptionalpropertytypes-2026-03-02)
42. [Route Module Extraction](#route-module-extraction-2026-03-02)
...
**Fix:** Grant camera permissions. Use Chrome or Safari for BarcodeDetector support. Ensure adequate lighting.
...
### Problem: Import errors after update — "Cannot find module"

**Cause:** `services/api.ts` (5,330 lines) split into 13 domain files. Update imports:
...
**Cause:** `exactOptionalPropertyTypes` enabled in `tsconfig.json`. Omit properties instead of setting to `undefined`.

---

## Route Module Extraction (2026-03-02)

### Problem: Routes not rendering or missing pages

**Cause:** Inline routes extracted from `App.tsx` into module `routes.tsx` files. Check `frontend/src/modules/<module>/routes.tsx`.

---

## Python Typing Modernization (2026-03-02)

### Problem: Backend type errors

**Cause:** `pyupgrade --py313-plus` applied to 56 files. Requires Python 3.10+ (project requires 3.13+).

---

## IP Spoofing & Security (2026-03-02)
...
**Cause:** Large components decomposed into sub-components:
