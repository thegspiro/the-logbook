...
- **Restock Reports** — *(2026-08-10)* A crew reports an item used or pulled *at the time they use it*, rather than leaving the gap for the next morning's check. Behind `equipment_check.submit`, the default member position. *(2026-08-11)* Withdrawing a restock report and swapping lots are corrections of record and require `equipment_check.manage` / `inventory.manage`
