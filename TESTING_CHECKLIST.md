...
| SKL-14 | Access skill testing without training.manage permission                     | Can start, score and complete tests (official and practice); **cannot** create/edit templates, validate, void or release _(revised 2026-08-08)_   |
| SKL-15 | Member (no training.manage) completes an official test                      | Result saved and scored, but `validated_at` is NULL — requirement NOT credited, no attempt spent, excluded from summary pass rate / average score |
| SKL-16 | Candidate views their own unvalidated official result                       | Listed as awaiting validation with the outcome withheld — no PASS/FAIL, no score                                                                  |
| SKL-17 | Officer completes an official test themselves                               | Validated in the same step — `validated_at` set, requirement credited immediately, never appears in the review queue                              |
| SKL-18 | Officer validates a member-run passing result                               | Requirement credited, one attempt spent, result enters pass rate/average, candidate sees it per the template's disclosure rule                    |
| SKL-19 | Officer validates the same result twice                                     | Idempotent — returns unchanged, no duplicate credit and no second audit entry                                                                     |
| SKL-20 | Officer validates a test they are the **candidate** in                      | Refused (400) — separation of duties                                                                                                              |
| SKL-21 | Validate a practice / voided / in-progress test                             | Refused (400) in all three cases with a distinct message each                                                                                     |
| SKL-22 | Officer voids a pending result (the rejection path)                         | Record kept with the reason and its author; candidate sees the reason; nothing credited                                                           |
| SKL-23 | Candidate search with `q` shorter than 2 chars, or whitespace only          | Refused (422); no request returns the full roster                                                                                                 |
| SKL-24 | Candidate search containing `%` or `_`                                      | Wildcards escaped — matches literally, does not return every member                                                                               |
| SKL-25 | Candidate search matching more than 15 members                              | Capped at 15, truncated **silently** (no indicator) — confirm the cap, not a message                                                              |
| SKL-26 | Candidate search as a member with neither training.view nor training.manage | Refused (403) — picker unavailable                                                                                                                |
| SKL-27 | Member starts a **practice** test                                           | Own name pre-filled as candidate, no lookup request issued                                                                                        |
| SKL-28 | Summary dashboard as a non-officer                                          | Aggregates visible; **Pending Validation reads 0** regardless of the real queue depth                                                             |
| SKL-29 | Upgrade an existing database with completed official results                | Migration `20260808_0001` backfills them as validated by their examiner — review queue is empty, not full of history                              |
| SKL-30 | Officer who validated a result is later deactivated/deleted                 | Result stays validated (`validated_by` SET NULL) — it does not revert to pending                                                                  |
