...
def upgrade() -> None:
    ...
    op.create_index(
        "idx_pipeline_org_default",
        "membership_pipelines",
        ["organization_id", "is_default"],
    )
    op.create_index(
        "idx_pipeline_org_template",
        "membership_pipelines",
    ...
    op.create_table(
        ...
        sa.Column(
            "pipeline_id",
            sa.String(36),
            sa.ForeignKey("membership_pipelines.id", ondelete="CASCADE"),
            nullable=False,
        ),
        sa.Column("name", sa.String(255), nullable=False),
        sa.Column("description", sa.Text),
        sa.Column(
            "step_type",
            sa.Enum("action", "checkbox", "note", name="pipelinesteptype"),
            nullable=False,
    ...
    op.create_table(
        ...
        sa.Column(
            "referred_by", sa.String(36), sa.ForeignKey("users.id", ondelete="SET NULL")
        ),
        sa.Column(
            "current_step_id",
            sa.String(36),
        ...
        sa.Column(
            "form_submission_id",
            sa.String(36),
            sa.ForeignKey("form_submissions.id", ondelete="SET NULL"),
        ),
        sa.Column(
            "transferred_user_id",
            sa.String(36),
    ...
    op.create_index(
        "idx_prospect_org_status", "prospective_members", ["organization_id", "status"]
    )
    op.create_index(
        "idx_prospect_org_pipeline",
        "prospective_members",
        ["organization_id", "pipeline_id"],
    )
    op.create_index(
        "idx_prospect_org_email", "prospective_members", ["organization_id", "email"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "step_id",
            sa.String(36),
            sa.ForeignKey("membership_pipeline_steps.id", ondelete="CASCADE"),
            nullable=False,
        ),
        sa.Column(
            "status",
    ...
    op.create_index(
        "idx_activity_log_prospect", "prospect_activity_log", ["prospect_id"]
    )
    op.create_index("idx_activity_log_action", "prospect_activity_log", ["action"])

    print(
        "Created membership pipeline tables: membership_pipelines, membership_pipeline_steps, prospective_members, prospect_step_progress, prospect_activity_log"
    )
