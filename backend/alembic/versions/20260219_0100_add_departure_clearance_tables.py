...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "value_outstanding", sa.Numeric(10, 2), nullable=False, server_default="0"
        ),
        sa.Column(
            "initiated_at", sa.DateTime(timezone=True), server_default=sa.func.now()
        ),
    ...
    op.create_index(
        "idx_departure_clearance_org_user",
        "departure_clearances",
        ["organization_id", "user_id"],
    )
    op.create_index(
        "idx_departure_clearance_org_status",
        "departure_clearances",
    ...
    op.create_table(
        ...
        sa.Column(
            "clearance_id",
            sa.String(36),
            sa.ForeignKey("departure_clearances.id", ondelete="CASCADE"),
            nullable=False,
        ),
        sa.Column(
            "organization_id",
    ...
    op.create_index(
        "idx_clearance_item_clearance", "departure_clearance_items", ["clearance_id"]
    )
    op.create_index(
        "idx_clearance_item_disposition",
        "departure_clearance_items",
        ["clearance_id", "disposition"],
    )
