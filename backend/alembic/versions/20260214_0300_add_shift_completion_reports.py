...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "trainee_acknowledged", sa.Boolean(), nullable=False, server_default="0"
        ),
    ...
    op.create_index(
        "idx_shift_report_trainee",
        "shift_completion_reports",
        ["trainee_id", "shift_date"],
    )
    op.create_index(
        "idx_shift_report_officer", "shift_completion_reports", ["officer_id"]
    )
    op.create_index(
        "idx_shift_report_enrollment", "shift_completion_reports", ["enrollment_id"]
    )
    op.create_index(
