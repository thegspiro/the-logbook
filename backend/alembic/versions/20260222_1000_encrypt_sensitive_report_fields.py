...
def upgrade() -> None:
    ...
    reports = conn.execute(
        sa.text(
            "SELECT id, officer_narrative, areas_of_strength, areas_for_improvement, reviewer_notes FROM shift_completion_reports"
        )
    ).fetchall()
    ...
    for row in reports:
        ...
        if updates:
            ...
            updates["id"] = (
                row._mapping.get("id") if hasattr(row, "_mapping") else row[0]
            )
            conn.execute(
                sa.text(
                    f"UPDATE shift_completion_reports SET {set_clause} WHERE id = :id"
                ),
                updates,
            )
