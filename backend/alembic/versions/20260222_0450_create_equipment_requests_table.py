...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "request_type", sa.String(20), nullable=False, server_default="checkout"
        ),
    ...
    op.create_index(
        "idx_equip_requests_org_status",
        "equipment_requests",
        ["organization_id", "status"],
    )
    op.create_index(
        "idx_equip_requests_requester", "equipment_requests", ["requester_id", "status"]
    )
