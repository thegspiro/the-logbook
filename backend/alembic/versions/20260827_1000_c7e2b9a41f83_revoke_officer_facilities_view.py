...
def upgrade() -> None:
    bind = op.get_bind()
    # `positions` is one of the tables no migration creates — it arrives with
    # create_all() at first boot. CI upgrades an empty database, so reflecting
    # it unconditionally would fail the whole chain.
