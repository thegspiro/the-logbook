...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "returned_by", sa.String(36), sa.ForeignKey("users.id"), nullable=True
        ),
    ...
    op.create_index(
        "idx_item_issuances_org_item", "item_issuances", ["organization_id", "item_id"]
    )
    op.create_index(
        "idx_item_issuances_org_user", "item_issuances", ["organization_id", "user_id"]
    )
    op.create_index(
        "idx_item_issuances_org_returned",
        "item_issuances",
...
def downgrade() -> None:
    ...
    op.drop_index(
        "idx_checkout_records_org_returned_expected", table_name="checkout_records"
    )
