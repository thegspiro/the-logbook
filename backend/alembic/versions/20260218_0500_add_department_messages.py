...
def upgrade() -> None:
    ...
    if not _table_exists(conn, "department_messages"):
        op.create_table(
            ...
            sa.Column(
                "priority",
                sa.Enum("normal", "important", "urgent", name="messagepriority"),
            ...
            sa.Column(
                "target_type",
                sa.Enum(
                    "all", "roles", "statuses", "members", name="messagetargettype"
                ),
                server_default="all",
                nullable=False,
            ),
        ...
        op.create_index(
            "idx_dept_msg_org_pinned",
            "department_messages",
            ["organization_id", "is_pinned"],
        )
    ...
    if not _table_exists(conn, "department_message_reads"):
        op.create_table(
            ...
            sa.Column(
                "message_id",
                sa.String(36),
                sa.ForeignKey("department_messages.id", ondelete="CASCADE"),
                nullable=False,
            ),
            sa.Column(
                "user_id",
            ...
            sa.Column(
                "read_at", sa.DateTime(timezone=True), server_default=sa.func.now()
            ),
            sa.Column("acknowledged_at", sa.DateTime(timezone=True), nullable=True),
            sa.UniqueConstraint("message_id", "user_id", name="uq_dept_msg_read_user"),
        )
        op.create_index(
            "idx_dept_msg_read_msg", "department_message_reads", ["message_id"]
        )
        op.create_index(
            "idx_dept_msg_read_user", "department_message_reads", ["user_id"]
        )
