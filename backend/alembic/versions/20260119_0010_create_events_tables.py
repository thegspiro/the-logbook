...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("id", sa.String(36), primary_key=True),
        sa.Column(
            "event_id", sa.String(36), sa.ForeignKey("events.id"), nullable=False
        ),
        sa.Column("user_id", sa.String(36), sa.ForeignKey("users.id"), nullable=False),
        sa.Column(
            "status",
            sa.Enum("going", "not_going", "maybe", name="rsvpstatus"),
            nullable=False,
    ...
    op.create_index(
        "ix_event_rsvps_event_user", "event_rsvps", ["event_id", "user_id"], unique=True
    )
