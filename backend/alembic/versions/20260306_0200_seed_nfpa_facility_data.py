...
def upgrade() -> None:
    ...
    result = conn.execute(
        text(
            "SELECT COUNT(*) FROM facility_types WHERE is_system = 1 AND organization_id IS NULL"
        )
    )
    ...
    result = conn.execute(
        text(
            "SELECT COUNT(*) FROM facility_statuses WHERE is_system = 1 AND organization_id IS NULL"
        )
    )
    ...
    result = conn.execute(
        text(
            "SELECT COUNT(*) FROM facility_maintenance_types WHERE is_system = 1 AND organization_id IS NULL"
        )
    )
