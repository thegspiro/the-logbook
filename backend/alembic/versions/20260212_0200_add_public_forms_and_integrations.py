...
def upgrade() -> None:
    ...
    op.add_column(
        "forms", sa.Column("public_slug", sa.String(12), unique=True, index=True)
    )
    ...
    op.add_column(
        "form_submissions",
        sa.Column("is_public_submission", sa.Boolean, server_default="0"),
    )
    op.add_column(
        "form_submissions",
        sa.Column("integration_processed", sa.Boolean, server_default="0"),
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "target_module",
            sa.Enum("membership", "inventory", name="integrationtarget"),
            nullable=False,
        ),
        sa.Column(
            "integration_type",
            sa.Enum(
                "membership_interest", "equipment_assignment", name="integrationtype"
            ),
            nullable=False,
        ),
        ...
        sa.UniqueConstraint(
            "form_id", "target_module", name="uq_form_integration_target"
        ),
    )
