...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "approval_deadline_days", sa.Integer(), nullable=False, server_default="14"
        ),
        # Notification settings
        sa.Column(
            "notify_officer_on_submit", sa.Boolean(), nullable=False, server_default="1"
        ),
        sa.Column(
            "notify_member_on_decision",
            sa.Boolean(),
            nullable=False,
            server_default="1",
        ),
    ...
    op.create_index(
        "idx_self_report_config_org", "self_report_configs", ["organization_id"]
    )
    ...
    op.create_index(
        "idx_submission_user", "training_submissions", ["submitted_by", "status"]
    )
