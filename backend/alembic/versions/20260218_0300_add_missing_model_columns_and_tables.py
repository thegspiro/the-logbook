...
def upgrade() -> None:
    ...
    if not _table_exists(conn, "event_templates"):
        op.create_table(
            ...
            sa.Column(
                ...
                sa.Enum("flexible", "strict", "window", name="checkinwindowtype_tmpl"),
                nullable=True,
            ),
            sa.Column(
        ...
        op.create_index(
            "ix_event_templates_organization_id", "event_templates", ["organization_id"]
        )
    ...
    if not _column_exists(conn, "membership_pipelines", "is_active"):
        ...
        op.create_index(
            "ix_membership_pipelines_is_active", "membership_pipelines", ["is_active"]
        )
    ...
    if not _table_exists(conn, "prospect_documents"):
        ...
        op.create_index(
            "idx_prospect_doc_prospect", "prospect_documents", ["prospect_id"]
        )
    ...
    if not _table_exists(conn, "prospect_election_packages"):
        ...
        op.create_index(
            "idx_election_pkg_prospect", "prospect_election_packages", ["prospect_id"]
        )
        op.create_index(
            "idx_election_pkg_status", "prospect_election_packages", ["status"]
        )
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_membership_pipelines_is_active", table_name="membership_pipelines"
    )
    ...
    op.drop_constraint("fk_events_recurrence_parent_id", "events", type_="foreignkey")
