...
def upgrade() -> None:
    ...
    op.create_index(
        "idx_category_org_code", "training_categories", ["organization_id", "code"]
    )
    op.create_index(
        "idx_category_parent", "training_categories", ["parent_category_id"]
    )
    ...
    op.add_column(
        ...
        sa.Column("due_date_type", sa.String(50), default="calendar_period"),
    )
    op.add_column(
        "training_requirements",
        sa.Column("rolling_period_months", sa.Integer, nullable=True),
    )
    op.add_column(
        "training_requirements", sa.Column("period_start_month", sa.Integer, default=1)
    )
    op.add_column(
        "training_requirements", sa.Column("period_start_day", sa.Integer, default=1)
    )
    op.add_column(
        "training_requirements", sa.Column("category_ids", sa.JSON, nullable=True)
    )
