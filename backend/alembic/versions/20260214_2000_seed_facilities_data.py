...
def downgrade() -> None:
    ...
    op.execute(
        "DELETE FROM facility_maintenance_types WHERE organization_id IS NULL AND is_system = 1"
    )
    op.execute(
        "DELETE FROM facility_statuses WHERE organization_id IS NULL AND is_system = 1"
    )
    op.execute(
        "DELETE FROM facility_types WHERE organization_id IS NULL AND is_system = 1"
    )
