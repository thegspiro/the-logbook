...
def upgrade() -> None:
    ...
    op.add_column(
        "elections",
        sa.Column("enable_runoffs", sa.Boolean(), nullable=False, server_default="0"),
    )
    op.add_column(
        "elections",
        sa.Column(
            ...
            sa.String(length=50),
            nullable=False,
            server_default="top_two",
        ),
    )
    op.add_column(
        "elections",
        sa.Column(
            "max_runoff_rounds", sa.Integer(), nullable=False, server_default="3"
        ),
    )
    op.add_column(
        "elections",
        sa.Column("is_runoff", sa.Boolean(), nullable=False, server_default="0"),
    )
    op.add_column(
        "elections", sa.Column("parent_election_id", sa.String(36), nullable=True)
    )
    op.add_column(
        "elections",
        sa.Column("runoff_round", sa.Integer(), nullable=False, server_default="0"),
    )

    # Add foreign key constraint for parent_election_id
    op.create_foreign_key(
        "fk_elections_parent_election_id",
        "elections",
        "elections",
        ["parent_election_id"],
        ["id"],
    ...
    op.create_index(
        "ix_elections_parent_election_id",
        "elections",
        ["parent_election_id"],
        unique=False,
    )
...
def downgrade() -> None:
    ...
    op.drop_constraint(
        "fk_elections_parent_election_id", "elections", type_="foreignkey"
    )
