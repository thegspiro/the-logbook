...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "exception_type",
            sa.Enum("allowlist", "blocklist", name="ipexceptiontype"),
            nullable=False,
            index=True,
        ),
        ...
        sa.Column(
            ...
            sa.Enum(
                ...
                "expired",
                "revoked",
                name="ipexceptionapprovalstatus",
            ),
            nullable=False,
            default="pending",
            index=True,
    ...
    op.create_index(
        "idx_ip_exception_type_status",
        "ip_exceptions",
        ["exception_type", "approval_status"],
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "exception_id",
            sa.String(36),
            sa.ForeignKey("ip_exceptions.id", ondelete="CASCADE"),
            nullable=False,
            index=True,
        ),
    ...
    op.create_index(
        "idx_exception_audit_exception", "ip_exception_audit_log", ["exception_id"]
    )
    op.create_index("idx_exception_audit_action", "ip_exception_audit_log", ["action"])
    op.create_index(
        "idx_exception_audit_time", "ip_exception_audit_log", ["performed_at"]
    )
    ...
    op.create_index(
        "idx_blocked_ip_time", "blocked_access_attempts", ["ip_address", "blocked_at"]
    )
    op.create_index(
        "idx_blocked_country_time",
        "blocked_access_attempts",
        ["country_code", "blocked_at"],
    )
