...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "requires_signature", sa.Boolean(), nullable=False, server_default="1"
        ),
    ...
    op.create_index(
        "idx_compliance_policies_org", "compliance_policies", ["organization_id"]
    )
    op.create_index(
    ...
    op.create_index(
        "idx_compliance_policies_active",
        "compliance_policies",
    ...
    op.create_index(
        "idx_policy_ack_unique",
        "policy_acknowledgments",
        ["policy_id", "user_id", "policy_version"],
        unique=True,
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "notify_on_failure", sa.Boolean(), nullable=False, server_default="1"
        ),
        sa.Column("active", sa.Boolean(), nullable=False, server_default="1"),
        sa.Column(
            "created_at",
    ...
    op.create_index(
        "idx_compliance_checklists_org", "compliance_checklists", ["organization_id"]
    )
    op.create_index(
        "idx_compliance_checklists_type",
        "compliance_checklists",
        ["organization_id", "checklist_type"],
    )
    ...
    op.create_index(
        "idx_checklist_sub_org", "checklist_submissions", ["organization_id"]
    )
    op.create_index(
        "idx_checklist_sub_checklist", "checklist_submissions", ["checklist_id"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "reported_by", sa.String(36), sa.ForeignKey("users.id"), nullable=False
        ),
        ...
        sa.Column(
            "status",
            sa.Enum(
                "open", "investigating", "resolved", "closed", name="incidentstatus"
            ),
            nullable=False,
            server_default="open",
        ),
    ...
    op.create_index(
        "idx_compliance_incidents_org", "compliance_incidents", ["organization_id"]
    )
    op.create_index(
        "idx_compliance_incidents_type",
        "compliance_incidents",
        ["organization_id", "incident_type"],
    )
    op.create_index(
        "idx_compliance_incidents_status",
        "compliance_incidents",
        ["organization_id", "status"],
    )
    op.create_index(
        "idx_compliance_incidents_date", "compliance_incidents", ["incident_date"]
    )
    op.create_index(
        "idx_compliance_incidents_severity",
        "compliance_incidents",
