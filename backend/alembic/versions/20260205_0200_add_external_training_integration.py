...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        ),  # vector_solutions, target_solutions, etc.
    ...
    op.create_index(
        "idx_ext_provider_org",
        "external_training_providers",
        ["organization_id", "active"],
    )
    op.create_index(
        "idx_ext_provider_type", "external_training_providers", ["provider_type"]
    )
    ...
    op.create_index(
        "idx_ext_mapping_provider", "external_category_mappings", ["provider_id"]
    )
    op.create_index(
        "idx_ext_mapping_external",
        "external_category_mappings",
        ["provider_id", "external_category_id"],
    )
    ...
    op.create_index(
        "idx_ext_user_external",
        "external_user_mappings",
        ["provider_id", "external_user_id"],
    )
    op.create_index(
        "idx_ext_user_internal", "external_user_mappings", ["internal_user_id"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "status", sa.String(50), default="pending", index=True
    ...
    op.create_table(
        ...
        sa.Column(
            "sync_log_id",
            sa.String(36),
            sa.ForeignKey("external_training_sync_logs.id", ondelete="SET NULL"),
            index=True,
        ),
        ...
        sa.Column(
            "import_status", sa.String(50), default="pending", index=True
    ...
    op.create_index(
        "idx_ext_import_provider",
        "external_training_imports",
        ["provider_id", "import_status"],
    )
    op.create_index(
        "idx_ext_import_external",
        "external_training_imports",
        ["provider_id", "external_record_id"],
    )
