...
def upgrade() -> None:
    ...
    op.add_column(
        "elections",
        sa.Column("quorum_type", sa.String(20), nullable=False, server_default="none"),
    )
    op.add_column("elections", sa.Column("quorum_value", sa.Integer(), nullable=True))
    op.add_column(
        "elections", sa.Column("last_chain_hash", sa.String(64), nullable=True)
    )
    ...
    print(
        "  - Added vote_dedup_hash (unique), chain_hash, receipt_hash, is_test to votes"
    )
