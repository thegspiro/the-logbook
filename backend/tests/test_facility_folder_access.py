...
def _folder(**kwargs):
    ...
    kwargs.setdefault("organization_id", "org-1")
    return SimpleNamespace(**kwargs)


class TestFacilityGateMatchesTheEndpoint:
    async def test_the_two_permission_sets_are_the_same(self):
...
class TestRequiredPermissions:
    async def test_holder_of_any_listed_grant_is_admitted(self):
        ...
        for grant in FACILITY_SENSITIVE_PERMISSIONS:
            assert await svc.can_access_folder(
                folder, "org-1", _user("documents.view", grant)
            ), grant

    async def test_documents_view_alone_is_refused(self):
        """The leak itself: a baseline member browsing the Documents module."""
        svc = DocumentsService(db=None)
        folder = _folder(required_permissions=list(FACILITY_SENSITIVE_PERMISSIONS))
        assert not await svc.can_access_folder(folder, "org-1", _user("documents.view"))

    async def test_leadership_does_not_override_it(self):
        ...
        svc = DocumentsService(db=None)
        folder = _folder(required_permissions=list(FACILITY_SENSITIVE_PERMISSIONS))
        assert not await svc.can_access_folder(
            folder, "org-1", _user("documents.manage")
        )
        assert not await svc.can_access_folder(folder, "org-1", _user("members.manage"))

    async def test_wildcard_still_admits(self):
        """A tenant administrator holding '*' holds every facilities grant by
        definition; refusing them would be a different bug."""
        svc = DocumentsService(db=None)
        folder = _folder(required_permissions=list(FACILITY_SENSITIVE_PERMISSIONS))
        assert await svc.can_access_folder(folder, "org-1", _user("*"))

    async def test_a_grant_held_through_operational_rank_admits(self):
        ...
        assert await svc.can_access_folder(folder, "org-1", chief)

    async def test_a_module_wildcard_admits(self):
        """`facilities.*` holds every facilities grant, but a raw set
        intersection matches none of them."""
        svc = DocumentsService(db=None)
        folder = _folder(required_permissions=list(FACILITY_SENSITIVE_PERMISSIONS))
        assert await svc.can_access_folder(folder, "org-1", _user("facilities.*"))

    async def test_absent_field_changes_nothing(self):
        """Every folder that predates this field must behave exactly as before."""
        svc = DocumentsService(db=None)
        assert await svc.can_access_folder(_folder(), "org-1", _user("documents.view"))
        assert await svc.can_access_folder(
            _folder(visibility=FolderVisibility.LEADERSHIP),
            "org-1",
            _user("documents.manage"),
        )
        assert not await svc.can_access_folder(
            _folder(visibility=FolderVisibility.LEADERSHIP),
            "org-1",
            _user("documents.view"),
        )

    async def test_it_composes_with_visibility_rather_than_replacing_it(self):
        ...
        assert not await svc.can_access_folder(
            folder, "org-1", _user("facilities.manage")
        )
...
class TestTheGateIsStampedOnTheFolderTreeItGuards:
    ...
    async def test_facility_sub_folders_are_stamped(self):
    ...
    async def test_the_apparatus_folders_already_stamped_are_repaired(self):
