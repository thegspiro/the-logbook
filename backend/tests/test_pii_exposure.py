...
class TestProfileEndpointAccessControl:
    ...
    off their own ID card. Viewing anyone else's full profile requires the
    stronger `users.view` or `members.manage` grant because the response also
    carries account state and role metadata.
    """
    ...
    async def test_members_view_cannot_read_other_full_profiles(self):
        subject = _member()
        caller = _caller(
            user_id=str(uuid.uuid4()),
            org_id=subject.organization_id,
