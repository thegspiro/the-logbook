...
# ---------------------------------------------------------------------------
# nginx API error responses
# ---------------------------------------------------------------------------

# Every nginx config that fronts the API, and the locations in each whose
# clients parse the response body as JSON rather than render it.
NGINX_JSON_ERROR_LOCATIONS = {
    "infrastructure/nginx/nginx.conf": ("/api/", "/api/v1/auth/login", "/health"),
    "frontend/nginx.conf": ("/api", "/health"),
}

NGINX_ERROR_STATUSES = (502, 503, 504)


def _nginx_conf_path(name: str) -> Path:
    return ROOT_DIR / name


def _nginx_block(content: str, header: str) -> str:
    """Return the body of the nginx block whose header matches ``header``.

    ``header`` is the text between ``location`` and ``{`` — the path or the
    named-location label — so a caller does not have to reproduce the exact
    whitespace of the file.
    """
    pattern = re.compile(
        r"^[ \t]*location[ \t]+" + re.escape(header) + r"[ \t]*\{", re.MULTILINE
    )
    match = pattern.search(content)
    assert match is not None, f"no 'location {header}' block found"
    ...
    for index in range(open_brace, len(content)):
        if content[index] == "{":
            ...
            depth -= 1
            if depth == 0:
                return content[open_brace + 1 : index]
    raise AssertionError(f"unbalanced braces in 'location {header}'")


class TestNginxApiErrorsAreJson:
    """An API client parses the body; it must never be handed the HTML page.

    limit_req's rejection is generated by nginx itself and never reaches the
    backend, so before this the only thing a throttled API or MCP client saw
    was /50x.html — which surfaces as a JSON parse error naming nothing.
    """

    def test_json_error_handlers_exist_and_keep_their_status(self):
        for name in NGINX_JSON_ERROR_LOCATIONS:
            conf = _read(_nginx_conf_path(name))
            for status in NGINX_ERROR_STATUSES:
                body = _nginx_block(conf, f"@api_error_{status}")
                assert (
                    "default_type application/json;" in body
                ), f"{name}: @api_error_{status} must send a JSON content type"
                # The status in the handler's own return, not merely any
                # return: three near-identical blocks invite a copy-paste
                # that would answer a 504 with a 502.
                assert re.search(
                    rf"\breturn\s+{status}\b", body
                ), f"{name}: @api_error_{status} must return status {status}"
                assert '"detail"' in body, (
                    f"{name}: @api_error_{status} must use the API's "
                    '{"detail": ...} error shape'
                )

    def test_api_locations_route_errors_to_the_json_handlers(self):
        for name, locations in NGINX_JSON_ERROR_LOCATIONS.items():
            conf = _read(_nginx_conf_path(name))
            for location in locations:
                body = _nginx_block(conf, location)
                for status in NGINX_ERROR_STATUSES:
                    assert f"error_page {status} @api_error_{status};" in body, (
                        f"{name}: 'location {location}' must map {status} to "
                        f"@api_error_{status}"
                    )

    def test_api_locations_never_serve_the_html_error_page(self):
        for name, locations in NGINX_JSON_ERROR_LOCATIONS.items():
            conf = _read(_nginx_conf_path(name))
            for location in locations:
                body = _nginx_block(conf, location)
                assert "50x.html" not in body, (
                    f"{name}: 'location {location}' serves an API client; it "
                    "must not fall back to the HTML error page"
                )
