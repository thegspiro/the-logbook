...
async def get_audit_log_entries(
    ...
    """
    # Tenant isolation: scope by the AuditLog.organization_id column — the
    # write-time org stamp — matching the canonical audit_logs.py endpoint.
    # (An earlier comment here wrongly claimed AuditLog has no organization_id
    # column; it does, and the user-id-membership subquery it justified diverged
    # from the canonical filter — it dropped org-stamped system rows with a NULL
    # user_id and resolved membership from the user's *current* org rather than
    # the row's stamp.) Without this, any org's leadership could read every
    # organization's audit trail.
    org_filter = AuditLog.organization_id == str(current_user.organization_id)

    query = select(AuditLog).where(org_filter).order_by(AuditLog.id.desc())
    count_query = select(func.count()).select_from(AuditLog).where(org_filter)
...
async def export_audit_logs(
    ...
    """
    # Tenant isolation: scope the export by the AuditLog.organization_id column
    # (see the entries handler above). Exporting the full cross-tenant audit
    # trail would leak every organization's usernames, IPs, and event payloads.
    org_filter = AuditLog.organization_id == str(current_user.organization_id)

    query = select(AuditLog).where(org_filter).order_by(AuditLog.id)
