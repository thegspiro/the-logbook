...
class LocationBase(BaseModel):
    ...
    name: str = Field(
        ...
        min_length=1,
        max_length=200,
        description="Location name (e.g., Main Meeting Hall)",
    )
    description: Optional[str] = Field(
        None, description="Additional details about the location"
    )
    ...
    floor: Optional[str] = Field(
        None, max_length=20, description="Floor number or name"
    )
    room_number: Optional[str] = Field(
        None, max_length=50, description="Room number or identifier"
    )
    capacity: Optional[int] = Field(None, ge=1, description="Maximum occupancy")
    facility_id: Optional[UUID] = Field(
        None, description="Link to Facility when Facilities module is enabled"
    )
    is_active: bool = Field(
        default=True, description="Whether location is available for events"
    )
...
class LocationDisplayInfo(BaseModel):
    ...
    location_name: str
    current_events: list = Field(
        default_factory=list, description="Events with active check-in windows"
    )
