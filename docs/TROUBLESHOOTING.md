...
1. **Not a permissions problem**: The widget reads `GET /notifications/my`, which is gated on authentication alone — every signed-in member can load their own notifications. The same is true of the Send Log tab, which reads `GET /notifications/logs` with its default `scope=mine`. Granting `notifications.view` will not fix an empty widget — it opens the department's notification rules, so grant it only to someone who should administer notifications. The organization-wide send log (`GET /notifications/logs?scope=organization`) is a separate grant again, `notifications.manage`.
...
## Mobile Member ID Scanner Issues

### Problem: "Scan Member ID" button not visible on mobile

**Status (Fixed 2026-03-02):** The mobile toolbar layout was updated so the "Scan Member ID" button is accessible alongside search and filter controls. If the button is hidden behind other elements, clear your browser cache and reload.
...
**Cause:** The member's ID card barcode or QR code must match a valid `membership_number` in the system.

**Fix:** Verify the member has a membership number assigned in their profile. If scanning a QR code from an ID card, ensure it was generated by The Logbook's ID card feature.

---

## API Service Split & Import Issues

### Problem: Import errors after updating — "Cannot find module 'services/api'"
...
### Problem: TypeScript errors about `undefined` not being assignable
...
## Route Module Extraction Issues

### Problem: Routes not rendering after update

**Cause (2026-03-02):** All inline route definitions were extracted from `App.tsx` into dedicated module `routes.tsx` files. If custom routes were added directly to `App.tsx`, they may have been lost.

**Fix:** Check if your routes are now defined in the appropriate module's `routes.tsx` file under `frontend/src/modules/<module>/routes.tsx`. New modules added: `action-items`, `admin`, `documents`, `forms`, `integrations`, `notifications`, `settings`.

### Problem: Lazy-loaded page shows loading spinner indefinitely

**Cause:** All module routes now use `lazyWithRetry()` for consistent chunk-loading. If the page component path changed during the extraction, the dynamic import may fail.

**Fix:** Check the browser console for chunk load errors. Clear browser cache with `Ctrl+Shift+R`. Verify the component path in the module's `routes.tsx` matches the actual file location.

---

## Python Typing Modernization Issues

### Problem: Backend type errors after updating

**Cause (2026-03-02):** Python typing was modernized across 56 backend files using `pyupgrade --py313-plus`. All `Optional[X]` was replaced with `X | None`, `List[X]` with `list[X]`, etc.

**Fix:** This requires Python 3.10+. If running an older Python version, these type annotations will cause `TypeError` at import time. Ensure your Python version is 3.13+ as specified in the project requirements.

---

## IP Spoofing & Security Middleware

### Problem: User IP addresses showing incorrectly in audit logs

**Status (Fixed 2026-03-02):** The security middleware was hardened to prevent IP spoofing via the `X-Forwarded-For` header. Only the rightmost untrusted IP is now used for rate limiting and audit logging.

**Impact:** If you use a reverse proxy (e.g., Nginx, Cloudflare), ensure it is configured as a trusted proxy. The middleware now correctly handles multi-hop proxy chains.

### Problem: Deprecated startup warnings in backend logs

**Status (Fixed 2026-03-02):** Deprecated `on_event("startup")` / `on_event("shutdown")` handlers replaced with the FastAPI lifespan context manager. No action required.

---

## Module Component Decomposition Issues
