...
**Amended 2026-08-23:** `equipment_check.submit` was added to that gate. The
finding was that the endpoint carried _no_ permission, and a submit-level gate
answers it — the caller still needs a check permission, the lot is still
org-scoped, every stored value still comes from the `InventoryLot` row rather
than the request, and `log_template_change` still records the author. The
manage-only reading cost more than it protected: replacing expired stock is
done by the crew at the compartment, and they could not, so the unit stayed
aboard and the item stayed force-failed until an officer was found. Withdrawing
a restock report (`DELETE /items/{id}/used`) remains manage-only.
...
apparatus is _supposed_ to be checked) that doesn't exist yet — an incomplete
