...
## Pass 5 (2026-08-14) — operational feature completeness

This pass followed the fleet-detail workflow rather than repeating the closed
FK and enum audits. It compared the apparatus model, schemas, API/service
surface, frontend types, detail page, and the still-open application guide
claims.

### AP2-4 — MED functionality — NFPA tracking is a checkbox with no usable workflow — 🚩 OPEN

The backend implements a complete NFPA-compliance resource: model rows carry
standard code/title, applicability, compliance status, assessment and due dates,
notes, and auditor metadata; the service and API expose list, detail, create,
update, and delete operations. The apparatus form also lets an administrator
enable `nfpa_tracking_enabled`.

The frontend stops there. It defines no NFPA-compliance type, service call,
store action, route, tab, or editor. When tracking is enabled, the Overview tab
only renders `Tracking Enabled`. An officer cannot add a standard, record an
assessment, see what is non-compliant, or act on an approaching due date. The
feature is therefore discoverable but non-functional from the product surface.

**Recommended fix:** add a permission-aware NFPA Compliance tab to apparatus
detail, backed by the existing API. Its default view should group overdue,
non-compliant, and upcoming records ahead of compliant records; management users
need create/edit/delete actions, while view-only users get the same operational
status without mutation controls.

**Acceptance criteria:** enabling tracking makes the tab available; an officer
can create and update a standard assessment; status and due dates survive a
reload; overdue/non-compliant items are visually and semantically distinct; all
states have loading/empty/error handling; and frontend contract tests cover the
API mapping and permission gates.

### AP2-5 — MED safety/UX — deficiency state has no age or source — 🚩 OPEN

The API and frontend types carry both `has_deficiency` and `deficiency_since`,
but list and detail views render only a generic `Deficiency` badge. The date is
never rendered, and there is no relationship or link to the failed equipment
check or maintenance record that caused the flag. A crew can tell that something
is wrong but cannot tell whether it was reported minutes or months ago, what
failed, or where to resolve it.

**Recommended fix:** first formalize deficiency provenance (`source_type` and
`source_id`, or a dedicated deficiency record) rather than guessing from the
latest failed check. Then replace the badge-only detail state with a prominent
banner containing the age, source summary, and a permission-safe link to the
originating check/maintenance item. Preserve a compact badge in fleet lists.

**Acceptance criteria:** every newly raised deficiency records provenance and a
timestamp atomically; apparatus detail shows both; authorized users can navigate
to the source; clearing a deficiency records who cleared it and why; historical
flags without provenance degrade honestly to “source unavailable”; and tests
cover raise, display, navigation, clear, and legacy rows.

### Additional opportunities

- Important-date cards show expiration dates but do not prioritize overdue or
  soon-due registration, inspection, insurance, and warranty states. A shared
  due-date presentation would turn passive data into an exception workflow.
- The maintenance/EVOC backend remains much broader than its focused integration
  coverage. Add MySQL-backed lifecycle tests for schedule → service → close and
  operator qualification/expiration before further expanding those areas.

No code fix was applied: AP2-4 is a frontend feature build, and AP2-5 requires a
provenance data-model decision. Rendering the existing date alone would improve
the badge but leave the more important “what failed?” workflow unresolved.
