...
| Last tended | 2026-08-25 — an 11-comment Codex review round caught real issues the "no new findings" draft missed. Fixed 9 in one tending pass: DOC-10 (legal effective dates shared one settings key between privacy/terms, now per-type), DOC-11 (malformed folder UUID escaped as 500, now a clean 400), DOC-12 (PATCH endpoints dropped explicit nulls via `exclude_none`, now `exclude_unset` + `apply_updates`), DOC-13 (upload's `name` was required but the UI advertises it optional, now derived from the filename), DOC-14 (station-print leaked printer host/port to non-admin holders, now a generic 502 + server-side log), DOC-15 (concurrent legal-document publishes could both succeed — added the organization-row lock + locking-read pair, Pitfall #27 shape), DOC-16 (a folder could be re-parented into its own descendant — added an ancestor-chain cycle check), DOC-17 (doc-only: route table used `/legal` instead of the real `/legal-documents` prefix). Flagged 2 unbounded-list findings (DOC-8 legal revision history, DOC-9 folder listing + N+1) consistent with this rotation's established pattern (FIN-9/ELEC-12/USR-5/MP-10/MS-6), mirrored to `KNOWN_LIMITATIONS.md`. **Next tending pass reconsidered DOC-18** (P1: no authenticated download endpoint existed for uploaded documents at all), initially flagged as a missing-feature/product-decision gap — on reflection `membership_pipeline.py::download_prospect_document` is an established precedent for exactly this shape (ACL-checked `GET .../{id}/download`, `FileResponse`, realpath containment check), so mirroring it is a small verifiable fix rather than new design; implemented, plus fixed the upload form's `folder: 'general'` sentinel default at its source (was still live after DOC-11's backend-only fix — a fresh org with no folders yet still hit the rejection on every upload). Also fixed `test_public_legal.py`, which the effective-date fix (DOC-10) had left referencing the old shared `lastUpdated` field. Two independent tending sessions worked this PR concurrently (a `git push` race); reconciled by taking the earlier push as base and layering the DOC-18 fix and test-file repair on top, rather than force-pushing over either side's work. Full completion gate re-run after all fixes (flake8/black/isort/migrations/137+ backend tests + frontend tsc/eslint, all green). See `docs/security-review/DOC-10-documents-legal.md` for the complete write-up and disposition of every finding. |
