...
| Threads     | 5 addressed (Codex review round on the draft: FAC-6/7/8/9 fixed, FAC-4 corrected), each replied to with the fixing commit and resolved.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| Last tended | 2026-08-26 — a Codex review round on the draft PR caught 5 real issues a "no new findings" first pass missed: **FAC-6** (HIGH — the new facility-folder bridge's get-or-create raced into a permanently-broken endpoint, fixed with an organization-row lock), **FAC-7** (MED — 19 update call sites hand-rolled a blind setattr, letting an explicit null on a NOT-NULL column 500 instead of 400, fixed by routing through the shared `apply_updates` utility), **FAC-8** (LOW — facility photo/document responses leaked the internal storage `file_path`, now excluded), **FAC-9** (LOW — the new folder endpoint's `document_count` crossed the `documents.view` boundary, now redacted for callers without it), and a stale FAC-4 claim (search actually is exposed) corrected in the module-audit/app-review docs. Pushed the fixes, guard tests added for all four code findings, full completion gate re-run (8317-test backend suite green). See `docs/security-review/FAC-12-facilities.md` for the complete write-up. |
