...
> **New release lesson:** [August 12–31, 2026 workflow updates](./19-august-2026-release-changes.md) covers changed workflows, permissions, data-sharing boundaries, edge cases, exact screenshot states, and required demo data across six change windows. **The August 24–31 section is the current one:** your department's **organizational chart** at Governance → Organizational Chart, open to every member and starting empty; a member record that now states **class and status as two separate facts** — which is why **life members and probationary members were being left off operational ballots** and now are not; the **equipment check template builder rebuilt as one list** you scroll, with the sidebar, progress strip, readiness card and Quick Add / Bulk Add toggle all gone; and the **Testing Checklist turned into a module that the upgrade switches off** — `/testing` disappears until an administrator turns it back on, and nothing recorded is lost. It also carries the upgrade's five **permission revocations**, the facility files that were readable by the whole department, the compliance setting that silently refused to clear (**re-check your compliance percentages**), and the grant reports that dropped everything recorded on their own end date. It names what is **not** in this release either: there is still no screen for entering member qualifications, and the equipment-check _lap_ remains built but unwired. **The August 23–24 section covers:** the **Schedule board** where a shift's chip says whether it still needs people and one button claims a seat, **standing shifts** ("every Tuesday night") that survive giving up a single date, **NFC ID cards** and an unattended **check-in station**, **network label printers** the server prints to directly — including the shift roster and the check sheet — **one shared frame** across the Members / Training / Inventory / Events administration pages, **sealed containers** in equipment checks (and the rule that a seal proves _unchanged, not full_), the rebuilt **Submit External Training** and **My Admin Hours**, and **five authorization fixes** — four of which an ordinary member could reach, including the assigned-gear table that rendered on every member profile. It also names what is **not** in this release: the equipment-check _lap_ is built but not wired. The August 19–23 section covers the new **Governance → Legal Documents** screen, the rule that you can no longer approve your own swap request, **paginated** swap/time-off requests, equipment checks that are safe to finish in a dead spot, a **Recruitment** event type, and the phone fix that makes dialog buttons tappable again. Earlier sections cover counting calls without an RMS, the three-step shift close-out, NFC tags for event / admin-hours / shift check-in, the `python -m app.preflight` pre-upgrade check, nested facility rooms, profile privacy, the security hardening batch, and the installation wizard's **one tab, one sitting** rule.
...
> property history intact. Audit-log retention is now enforced rather than
> merely configured. See also the new privacy sections in
...
>
> **Note (2026-07-14):** Training programs ("pipelines") now work as a full enroll → progress → advance → complete loop. The create-pipeline wizard builds a program atomically (program code, phases with optional "require officer approval to advance", requirements, milestones). The program's Enrollments tab lists members by name with progress and enrolls via a searchable member picker; bulk enroll blocks members failing prerequisite/concurrency rules. Officers track each member's requirements (log hours/shifts/calls/courses, mark complete/verify); completing any requirement type counts toward the overall percentage. Phases gate advancement (auto through completed phases, manual "Advance to next phase" when approval is required) with notifications. Progress is fed automatically by shift completion, by approving a program-linked training session (by requirement or by category), and by passing a linked skills test. Knowledge tests support officer-entered pass/fail or score % (passing score default 70%, max attempts enforced). Members see their own progression at **My Training → View full progress** (current phase, overall %, milestones, requirements grouped by phase). RSVP / self check-in to a session ahead of a member's current phase shows a soft, overridable warning. A full online knowledge-test-taking engine remains future work.
>
...
>
> **Note (2026-03-31):** Shift completion pipeline with end-to-end finalization workflow: `POST /scheduling/shifts/{id}/finalize` snapshots call_count and total_hours, computes per-member call counts, auto-creates draft ShiftCompletionReports for enrolled trainees. Pre-finalization checklist validates equipment checks. Finalized shifts protected from edit/delete. Auto-populate report data from shift records (hours, calls, call types) with audit trail in data_sources JSON. Draft → approved transition triggers deferred pipeline progress. Officer analytics dashboard with org-wide totals, per-trainee breakdown, status counts, and monthly trends. Trainee stats with personal totals and monthly breakdown. Report review workflow with approve/flag/redact and trainee acknowledgment. Encrypted evaluation fields (AES-256). ShiftReportsTab expanded to 5 view modes. Elections module: N+1 query fixes, typed ForensicsResponse schema, new /send-report and /verify-receipt endpoints. HIPAA audit logging added to medical screening, documents, membership pipeline, and messages. Pagination standardized across 16 previously unbounded endpoints. XSS prevention via React-based SimpleMarkdown component replacing dangerouslySetInnerHTML. Frontend pattern consolidation: useEmailListInput shared hook, APIKeysTab modal migration, PipelineTable shared SortableHeader. Security fix: analytics user_id spoofing patched. Form value coercion: ~15 instances of `??` → `||` fixed. Database indexes added on users table.
>
> **Note (2026-03-26):** Notification cards redesigned with expand/collapse UI, pinned-first sorting, contextual CTAs ("View Shift", "Start Checklist"), and mark-as-read on collapse. Notification metadata JSON column added for rich rendering. Scheduling page supports `?tab=` deep-linking (schedule, my-shifts, open-shifts, requests, equipment-checks). Shift notifications deep-link to scheduling with shift pre-selected. In-process scheduled task runner replaces external cron. Standalone equipment checks (not tied to shifts) with flat scrollable form and inline compartment headers. Text check type changed to read-only statement. Section headers (`is_header`) for visual grouping in templates. Critical minimum quantity threshold on check items. Template clone preserves is_header and critical_minimum_quantity. EVOC certification levels (Basic/Intermediate/Advanced) integrated across training, apparatus, and scheduling. Training record category tracking (Fire, EMS, Hazmat, Rescue). Virginia NCCR recertification standards. Event attendees importable into election ballot lists. Linked elections displayed on event and minutes detail pages. Apparatus type/status badges render actual icons. navigate(-1) replaced with hardcoded back paths and breadcrumbs. Chrome label printing fixed via iframe-based approach. App startup handles MySQL not ready with retry backoff.
>
> **Note (2026-03-24):** Scheduling module adds bulk confirm/decline on My Shifts, inline approve/deny on Requests, staffing status visualization (green/amber tints, ratio display), position-first assignment flow with "Fill All Open" bulk assign, unavailable member filtering, required/optional position toggle on templates, shift assignment notifications, and start-of-shift reminders with equipment checklists. Elections module gains tabbed secretary workflow (ElectionWorkflowTabs), eligibility roster with search/filter/expandable rows, publish results panel, runoff chain timeline, summary dashboard cards, and 5 new enums extracted to constants. Inventory module shows items in storage areas with expandable panels, always-visible barcode/asset tag fields, lazy barcode backfill, and WebSocket double-accept fix. Notifications overhauled: batch mark-all-read, unread badges on bell/side nav, smart polling (Page Visibility API), pagination, read/unread filter, dashboard clear-all fix, clickable notifications, View All link fix. Membership adds department email generation (4 format patterns, collision handling), username collision prevention, default member role, password_changed_at on creation, and membership ID auto-generation with number reuse safety. WCAG accessibility: color contrast fixes across 75 components, new colorContrast.ts utility, form accessibility (htmlFor/id, aria-required, fieldset/legend), aria-live on 52 elements, camera error handling improvements. 10 scheduling bug fixes including overlap detection, UTC in notifications, color parsing, member hours report, and dark mode contrast.
>
> **Note (2026-03-23):** Camera scanning now works on all modern browsers (Firefox, Safari, desktop). The InventoryScanModal falls back to html5-qrcode when native BarcodeDetector API is unavailable. Desktop webcam support via environment→user camera fallback. Scanner logic extracted into shared modules (`useHtml5Scanner` hook, `scanner.ts` types, `camera.ts` constants). Dashboard notifications can be dismissed individually or cleared in bulk; persistent department messages require admin clearance. Notifications page renamed to "Notification Rules & Logs" with channel filtering. Inventory admin hub redesigned with grouped sections. New admin pages for Equipment Kits (`/inventory/admin/kits`) and Variant Groups (`/inventory/admin/variant-groups`). Scheduling permissions fixed: `scheduling.assign` for assignments, `scheduling.manage` for shift CRUD. Calls/Incidents placeholder removed from shift detail panel.
>
