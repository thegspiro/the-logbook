...
> **New release lesson:** [August 12–23, 2026 workflow updates](./19-august-2026-release-changes.md) covers changed workflows, permissions, data-sharing boundaries, edge cases, exact screenshot states, and required demo data across four change windows. **The August 19–23 section is the current one:** the new **Governance → Legal Documents** screen (write your own privacy notice, with publishing held behind a second permission), the rule that **you can no longer approve your own swap request** — which will strand a department where exactly one person holds `scheduling.manage` — **paginated** swap/time-off requests (a breaking change for any script reading those endpoints), **equipment checks that are safe to finish in a dead spot**, a **Recruitment** event type that feeds the prospect pipeline, and the phone fix that makes **dialog buttons tappable again**. The August 17–19 section covers: **counting calls without an RMS** (and why the report says _unit responses_, which matters before anyone quotes the figure in a grant application), the **three-step shift close-out** that saves as it goes, **NFC tags** for event / admin-hours / shift check-in, and the `python -m app.preflight` check to run _before_ an upgrade rather than after it fails. Earlier sections cover nested facility rooms, candidate-list visibility, profile privacy, the security hardening batch, and the August 15 change that makes the installation wizard a **one tab, one sitting** operation.
