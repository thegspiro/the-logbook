...
The wizard opens on **Setup Prerequisites** (`/onboarding/prepare`), which
collects nothing: it lists what setup is going to want, split into what is
required and what can be skipped — SMTP credentials, an OAuth client secret, a
storage key — so you can gather them before you start.
...
Eleven steps follow. **Only the first two are required**, and the progress strip
marks the optional ones from the start:
...
The order is deliberate: **identity second**, so everything after it belongs to
a real signed-in account; then **what the department uses** — modules, ranks,
stations, apparatus; then the **external integrations** — email, storage,
sign-in — which are the steps that send somebody off to find credentials, and
every one of which can be skipped.
...
Three settings are worth answering during setup rather than afterwards.

**How you number members**, in step 1. The counter only numbers members created
**after** it is switched on, and the wizard creates your administrator account
in step 2 and your IT team in step 7. Switch it on later and those first
accounts hold no number, and a roster import starts at the number they should
have had.
...
**Your rank ladder**, in step 4: rename ranks to your own vocabulary, reorder the
ladder, remove ranks you do not have, add your own, and set which shift seats
each rank can fill — including a seat your department invented.
...
**Your membership tier ladder**, also in step 4 — the same editor as **Members
Administration → Settings → Membership Tiers**.
...
Step 4's position permissions **show rows only for the modules you turned on in
step 3** — which is why Modules comes first — with **Show all modules** to see
the rest. Every checkbox grants a permission that exists, and unticking one on a
seeded position removes it.
...
> Screens a manager needs to read accept the manage permission as well, but on a
> position you build yourself, tick both if its holder should read and manage.
...
| Scheduled emails not sending                          | **First check for an enabled-but-incomplete organization section** — an enabled section short-circuits the deployment's `SMTP_*` settings whether or not it can send, so a half-filled Cloudflare section — or the platform left at **Not configured** with email enabled — silently stops a department that was sending through the server's own configuration. Complete it or disable it. _(2026-09-13)_ Otherwise: verify SMTP is configured in Settings > Email, and that the background email scheduler is running (polls every 60 seconds). For Gmail or Microsoft 365, choose that platform — its host, port and encryption come from a preset — enter an app password, and press **Test Connection**. |
...
The credential fields are per-platform: pick **Cloudflare** and the Account ID and API Token appear beneath the sender fields. **Test Connection** signs in to the provider with what is on the form, without saving it. For a test message end to end, use the test send in the email template editor, which uses whatever platform is saved here.

**Gmail and Microsoft 365** are SMTP submission behind an app password: you
enter the From address and the app password, and the host, port, encryption and
login come from a fixed preset that both sending and **Test Connection** use.

**Microsoft 365** also offers **App registration (OAuth)**, which collects the
directory (tenant) ID, application (client) ID and client secret. The app
registration needs the `SMTP.SendAsApp` application permission and `SendAs` on
the sending mailbox; a token Exchange Online refuses is reported as a mailbox
grant problem, not as bad credentials, because the two are fixed in different
places. Choosing Microsoft 365 afresh preselects OAuth; a configuration already
using an App Password keeps it.

> **An App Password is Basic authentication, and Microsoft is retiring it for
> SMTP.** Unchanged through December 2026, disabled by default for existing
> tenants at the end of it, unavailable to tenants created after, and removed in
> the second half of 2027. Plan the move to App registration before then.
...
_The success message in this picture is simulated. The demo department has no
Microsoft 365 tenant, so the test request was answered with the message a real
App Password test returns when it succeeds, **SMTP connection successful**.
Everything else is the real screen, and nothing on it was saved._

---

## An enabled email section that cannot send is refused _(2026-09-13)_

**Read this if your department's mail has ever gone quiet for no reason you
could find.** It is the most consequential email change in months, and the
symptom is silence rather than an error.

### What used to happen

Settings → Email would save an **enabled** section green that had no chance of
delivering a message. Two shapes did it:

- **Cloudflare** selected with no Account ID or API Token. The Cloudflare sender
  needs all three of account, token and From address; without them the platform
  falls through to the SMTP path, and a Cloudflare row carries no host. Every
  send failed with _"SMTP host and from_email are required"_.
- **Not configured** selected — the button labelled **Other** until
  2026-09-13. **It was never a catch-all**: it stores nothing and there is no
  transport behind it. Enabling email while it is chosen is not an incomplete
  configuration but an impossible one, which is what the rename now says out
  loud.

### Why it was worse than a validation gap

**An enabled organization section short-circuits the deployment's own SMTP
settings.** The sender checks your organization's section first, and returns as
soon as it finds one enabled — it never looks at the server-level `SMTP_*`
configuration behind it.

So a hollow section did not merely fail on its own. **It stopped whatever
deployment-wide mail configuration was working from being used at all.** A
department sending perfectly well through the server's SMTP, whose administrator
then came in and half-filled an email section, stopped sending — with a green
"Email settings saved" toast and no error anywhere in the application.

### What happens now

The missing field is named on the write, and the settings screen says so inline
rather than leaving Save to explain it. A **present but misshapen** Cloudflare
Account ID is rejected here too: it must be the 32-character hexadecimal value
from your Cloudflare dashboard, not the account name.

**If your mail stopped and you never worked out why, this is the first place to
look.** Open Settings → Email and either complete the section or **turn it
off**. A disabled section hands sending back to the deployment's own
configuration.

| Scenario                                            | Behavior                                                                                |
| --------------------------------------------------- | --------------------------------------------------------------------------------------- |
| Cloudflare enabled, no Account ID or API Token      | Save refused, naming the missing field                                                  |
| Cloudflare Account ID that is not 32 hex characters | Save refused with the expected format, rather than failing at send time                 |
| **Not configured** chosen, email enabled            | Save refused — the platform stores nothing and cannot send                              |
| Section **disabled**                                | Sending falls back to the deployment's `SMTP_*` (or `CLOUDFLARE_*`) settings            |
| A stored `email_service` of `null`                  | No longer breaks sending. The settings screen always read it safely; the sender did not |

## Two platform buttons were renamed _(2026-09-13)_

| Was                  | Is now                  |
| -------------------- | ----------------------- |
| **Self-Hosted SMTP** | **SMTP (any provider)** |
| **Other**            | **Not configured**      |

**The rename is the fix, not decoration**, and it is why the rest of this
section reads the way it does. "Self-Hosted SMTP — your own mail server" told a
department running on Yahoo that the platform did not support them; "Other"
read like a catch-all for anything unlisted. Both names pointed departments at
the one option that cannot send. The new names say what each button does: one
takes any SMTP server you can name, the other records that you have not set
email up.

Older captures and older recordings show the previous names. That is the only
thing about them that is wrong — the panels beneath each button are unchanged.

## Twelve SMTP quick-fill presets _(2026-09-13)_

Choosing **SMTP (any provider)** now offers a preset list: **Yahoo, iCloud,
Zoho, Fastmail, AOL, GMX, SendGrid, Amazon SES, Mailgun, Postmark, Brevo** and
**Mailjet**. Picking one fills in the host, port and encryption that provider
documents, and tells you what its Username and password fields expect.

**Most of them want an app password, not your account password** — generated in
the provider's own security settings once two-factor sign-in is on. That is far
and away the commonest reason a correct host and port still fails to
authenticate.

**This is a labelling fix, not a new capability.** All twelve are ordinary SMTP
submission, which the platform has always sent perfectly well; what departments
did not have was any way to know that.

The stored shape is unchanged — still a self-hosted configuration with the same
fields — so an existing SMTP setup is untouched, and the three values a preset
fills in remain editable.

**The provider list is a one-shot chooser, not a stored setting.** It resets to
_Choose a provider…_ as soon as it has filled the three fields, which is why the
line naming your provider and its credential rule sits underneath rather than in
the control. Nothing records which preset you used, and nothing needs to.

> **Proton Mail Bridge is deliberately absent.** It listens on `127.0.0.1:1025`
> with a self-signed certificate, and the sender verifies certificates, so the
> handshake fails every time. A preset that cannot connect would read as a
> supported provider, which is worse than no preset.
...
## Two Cloudflare corrections _(2026-09-15)_

- **Ballot email now sends.** The election ballot fan-out is the platform's one
  batch sender, and it handed Cloudflare raw message data the Cloudflare API
  does not accept — so it warned about falling back to SMTP and fell back to a
  host that does not exist for a Cloudflare-only organization. Departments on
  Cloudflare could not send ballot notices at all.
- **A deployment-wide Cloudflare account is a default, not an override.** With
  `CLOUDFLARE_EMAIL_ENABLED` set, the global account was used for **every**
  organization, including one running its own enabled SMTP — so mail went out
  through an unrelated account under the deployment's sender address. An
  organization that has configured its own transport is no longer a candidate
  for it.

Both Cloudflare paths also now carry the **List-Unsubscribe** headers the SMTP
path always set. Gmail and Yahoo require one-click unsubscribe from bulk
senders, so the same ballot notice was being treated differently depending on
which backend sent it.
...
| `notifications.view`  | **Revoked** from the baseline member and junior-rank positions   | It gates the Notification Rules tab. (The Send Log is now every member's own delivery history, open to everyone.)                                                                                                                                                                                                                                                            |
...
The cause: the old setup screen's position editor saved its own checkbox
defaults over the seeded positions, so every department that finished setup
held grants the registry never gave them. The
[upgrade note](../UPGRADING.md#six-upgrade-steps-take-permissions-away-2026-09-05)
lists everything removed.
