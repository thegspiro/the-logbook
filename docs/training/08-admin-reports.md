...
| Scheduled emails not sending                          | **First check for an enabled-but-incomplete organization section** — an enabled section short-circuits the deployment's `SMTP_*` settings whether or not it can send, so a half-filled Cloudflare or "Other" section silently stops a department that was sending through the server's own configuration. Complete it or disable it. _(2026-09-13)_ Otherwise: verify SMTP is configured in Settings > Email, and that the background email scheduler is running (polls every 60 seconds). For Gmail, use STARTTLS on port 587 with an app password. _(fixed 2026-03-13)_ |
...
- **Other** selected. **"Other" is not a catch-all** — it stores nothing and
  there is no transport behind it. Enabling it is not an incomplete
  configuration but an impossible one.
...
| **Other** enabled                                   | Save refused — the platform stores nothing and cannot send                              |
...
Choosing **Self-Hosted SMTP** now offers a preset list: **Yahoo, iCloud, Zoho,
Fastmail, AOL, GMX, SendGrid, Amazon SES, Mailgun, Postmark, Brevo** and
...
did not have was any way to know that. A chief whose department runs on Yahoo
read "Self-Hosted SMTP — your own mail server", reasonably concluded the
platform did not support them, and picked **Other**, which cannot send at all.
...
> **Screenshot needed:**
> _[Settings → Email with Self-Hosted SMTP chosen and the preset list open,
> showing the twelve providers. Replace any capture of the platform list taken
> before September 13.]_
