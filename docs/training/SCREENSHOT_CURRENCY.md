...
`prettier --write` over five of the training guides removed 40 tracked
screenshot requests from `SCREENSHOT_STATUS.md`** — and changed nothing a
reader would notice.
...
**Fixed** by adding the five affected guides to `.prettierignore`, with the
reasoning written there. The five were identified empirically, by running
Prettier over a copy of every guide; they are the only ones affected. Re-run
that check before adding a guide to the list, and take a guide off the list
once its markers are captured.
