...
| Printing with "Background graphics" on _(2026-08-15)_       | Browsers do not print page backgrounds by default. If you have switched that option on **and are in light mode**, check one page before running a batch — the app's themed background may print behind the sheet. (Dark mode is unaffected, for an incidental reason recorded in [Known Limitations](../KNOWN_LIMITATIONS.md).) The same page also shows a grey backdrop on screen that no longer covers the full window; that is cosmetic and does not affect what prints |
