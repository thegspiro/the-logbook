...
| Ballot email link points to the wrong site or won't load | The emailed link is built from the server's `FRONTEND_URL` setting, not the request URL. Have your administrator set `FRONTEND_URL` to the real public site URL (a `localhost` value is flagged by a startup warning) and re-send the ballots.                                               |
