...
Every station and room gets a kiosk **display code** when it is created, and its QR code opens the room's public kiosk display (`/display/{code}`) for event check-in. The **Room QR Codes** page (`/locations/qr-codes`) collects every code in one printable directory, grouped by station/facility, with a search box for large departments. Two print layouts: **Cut-out cards** (a compact sheet — print, cut out, and post each code) and **Room signs** (one full-page sign per room, sized for posting on a door). Reach it from the **Room QR Codes** button on the Locations page, the **Room QR Codes** link in a facility's Rooms section, or the command palette (Ctrl/Cmd+K, search "QR"). Rooms in a facility's Rooms section also have their own QR button to view and copy a single room's code in place.
