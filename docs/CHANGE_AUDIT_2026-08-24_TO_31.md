...
1. **The Testing Checklist is now a module, and it is off.** `/testing` was
   ...
   upgrade. **No marks are lost** — the rows stay and reappear when the
   module is switched back on.

2. **Five migrations take permissions away from seeded positions, and one
   clears a stored field.** `compliance.view` off the Member position,
...
| **Equipment-check template builder — one canvas**               | ≈35 PRs across the `codex/*` equipment-check branches (#1976–#2062) and `claude/claude-design-changes` (#2064, #2068, #2071); revision tracking #2005 | Scheduling → equipment check templates: the builder                                                  | `equipment_check_templates.content_revision`; new table `equipment_check_bulk_delete_requests`; bulk-delete, clone and reorder endpoints                                                          | Sections, locations and items are now rows in **one list, in the order a crew walks them**. Name, answer type (Works / Count / Level / Date) and the one number that answer is graded against are edited in the row; description, serials, image, critical minimum and the inventory link moved behind a per-row disclosure, so opening anything is no longer a prerequisite for a complete item. Nesting is an indent button, not a dropdown listing every other location. **The reason Publish is unavailable is a list of specific fixes** that jump to the offending row, open its location and put the cursor in the empty field. The mobile preview docks beside the checklist on laptops and tablets; on phones it stays a modal. **Phones keep the compact rows and the full-height item editor** — the side-by-side row controls are a laptop/tablet layout. Bulk deletes go through a **retry-safe ledger** so a repeated request cannot delete rows a later edit added                                                                                                                                       |
...
Five of the forty-five are **merge revisions** that exist only to rejoin
...
| -------------- | --------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `a7c4e9b13f58` | Clears `User.rank` on every administrative member               | An operational rank is a chain-of-command position, and `_collect_user_permissions` unions `get_rank_default_permissions(user.rank)` into a member's effective permissions — so an administrative Fire Chief held operational grants. **Nothing records which ranks this cleared**, so restoring them would restore ranks an officer may have cleared deliberately |
...
linked to a position fills its holders from that position's assignees, and
unlinking leaves the holders in place.
...
- **NULL means "nobody has set this"**, and readers fall back to deriving from
  the legacy field. ORM writes never leave it NULL.
...
**This is the section to read before upgrading a live department.** Five
migrations move grants on _seeded_ positions. Scoped to `is_system = True` — a
...
| `e3b7c25f9a41`                                 | **Grants** `training.configure` to the seeded positions that configure training     | New permission. It gates the training module's org-level settings — chiefly the member visibility panel, which decides how much of an officer's written assessment the assessed member may read. That was gated on `training.manage`, which also carries the power to edit anybody's training records; a Membership Coordinator needs the first without the second |
...
Documents (see the release map). Every member could read every other member's
notifications. A membership applicant's file could reach a signer who could
not otherwise view it. Six pages could hold member-identifying data in the
browser's stale-while-revalidate cache for up to 90 seconds — a training
cohort roster, a program's enrollment-eligibility list, an external provider's
member mappings, a form-management page, a raw analytics export, and two
training-officer dashboards.
...
### QUAL-1 — qualification entry has no admin screen

`member_qualifications`, its service and its shift-eligibility reader all
shipped. **No screen writes to the table.** Recorded in full as
[QUAL-1](./KNOWN_LIMITATIONS.md#qual-1--member_qualifications-ships-with-no-officer-facing-entry-screen-2026-08-26).

The reason it is worth an entry rather than a to-do: everything about the
feature reads as complete — model, service, tests, eligibility integration,
changelog entry — so the natural next step is to document and narrate
qualification entry as something an officer can do. It is not, and the
do-not-capture notes in this audit, `training/01-membership.md` and
`training/SCREENSHOT_CURRENCY.md` exist to stop that.
...
1. **The equipment-check template builder.** Every capture of that screen
   ...
   side-by-side row controls are a laptop/tablet layout) _and_ at a phone
   width (compact rows, full-height item editor, blockers as a bottom sheet).
2. **Anything showing a member's membership type.** Member profile, Members
   administration, the member create/edit form and the elections eligibility
   panel now show **class and status as separate fields**. A capture showing
   one "Membership type" dropdown is wrong.
...
| Member **profile** header                                                 | **REPLACE**               | Class and status are separate fields                                                            |
| Member **create / edit** form                                             | **REPLACE**               | Same, plus the restored password / roles / welcome-email / address / emergency-contact fields   |
| **Members administration** list                                           | **REPLACE**               | Class and status columns                                                                        |
| Elections **eligibility** panel                                           | **REPLACE**               | Operational standing now reads class, so life members and probationers appear                   |
...
| **Member qualifications** entry                                           | **DO NOT CAPTURE**        | No admin screen exists (see Known limitations)                                                  |
...
- **04 — Fire Chief / Leadership.** The org chart is a chief's screen and has
  ...
  can open.
- **05 / 16 — Training Officer.** `training.configure` splits the visibility
  panel away from `training.manage`; any take saying the panel is behind
  "training management" is **wrong**.
