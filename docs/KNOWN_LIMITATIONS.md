...
| **`RateLimiter` tracks per-key state across three independent dicts (plus one per-scope dict) — collapse to one per-key record: recommended next priority on this file, confirmed by a seventh round** | 🚩 Open (design follow-up — RECOMMENDED NEXT PRIORITY; 2026-09-07, security review CI3-33; recurrence confirmed 2026-09-07 PR #2370 round 5; round-6 trigger met 2026-09-07 PR #2370 round 6; round 7 (CI3-33-2d) is the confirmation round 6's write-up said a further round would be) | `self.requests`, `self.lockouts`, and `self._key_windows` (`security_middleware.py`) are three separately-capped, separately-evicted structures meant to describe the same key's state, and seven successive Codex review rounds across two PRs (#2368, then #2370 — the latter covering three Codex rounds, since #2370's own fixes each drew another round of review before the PR could land) each found a new way they — or a new mechanism added to patch a prior round — drift or misbehave: CI3-33-1/2 (forced eviction silently lifting an active lockout / undercounting a key's own history), CI3-33-1a (window metadata not restored alongside history), CI3-33-1b/1c (`self.lockouts` uncapped, then its by-expiry eviction itself releasing an unrelated key's active lockout), CI3-33-1d/1e (`self._key_windows` uncapped independent of the other two; a saturated-table violator's only remaining protection being the much shorter request-history window rather than its lockout duration), CI3-33-2a/2b (round 5: the _new_ saturation-reject signal CI3-33-1e added was a single global scalar instead of being scoped per rate-limit scope — a cross-scope DoS — and a strict `>` eviction-gate comparison let a stale, already-expired lockout count trigger an unnecessary saturation rejection), CI3-33-2c (round 6: CI3-33-2b's own fix — forcing an immediate sweep once `self.lockouts` merely reached capacity — turned every request sharing this limiter into a full three-dict scan for as long as an attacker kept the table full, a CPU-amplification DoS), and **CI3-33-2d (round 7: CI3-33-2c's own scoping of that same sweep to "the one call that needs an accurate answer" still let an already-saturated key's own repeated retries each pay the full scan, since a caller controlling its own retry rate makes "one call" mean as many calls as it chooses)**. All seven were fixed narrowly and are covered by regression tests. Round 6's write-up said explicitly: "if another round of this general shape is found before the refactor lands, that is no longer a data point to weigh; it is confirmation the call made here was right" — round 7 is that confirmation, not a new data point, and does not change the recommendation further: **the refactor should be the next piece of work on this file, not an indefinitely-deferred design item.** Recommended shape: collapse to a single dict keyed by `key`, holding `{requests: list[float], window_seconds: int, lockout_until: float \| None}`, plus keep the per-scope saturation signal as a separate small dict (its key space is fundamentally different — a fixed set of code-literal scopes, not attacker-influenceable identifiers) — one eviction/expiry path instead of several that can disagree, closing this defect class by construction. Not attempted in either PR so far: every fix has landed mid-incident against a live Codex round, and the `TestRateLimiter` suite (35 tests as of PR #2370 round 3) constructs its scenarios by writing directly into the internal dicts, so this remains a deliberate, separately-reviewed and tested change, not a drive-by refactor folded into a bug-fix commit — the reasoning for keeping each fix itself narrow is unchanged even though the recommendation for what happens next is not. See `docs/security-review/CI3-33-core-infra.md` (CI3-33-2d's "On the structural refactor" section). |
