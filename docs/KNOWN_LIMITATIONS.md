...
| **Grants: applications list still truncates past 1,000 rows in a single status, with no pagination UI**                              | Open (LOW, real feature work — pagination UI or full-fetch looping)       | `GrantApplicationsPage.tsx` fetches with `limit: 1000` (the backend's own `PaginationParams` ceiling) rather than the server's 100-row default, closing the more likely case (GF-31/GF-32) of losing rows past the newest 100. A department with more than 1,000 applications in one pipeline status would still lose the oldest ones, silently — the page has no pagination controls in either its pipeline (kanban) or table view, both designed to render the full result set in one pass. Closing it needs either a pagination UI (a real product/design change for a page built around "show everything") or looping the fetch across `skip`/`limit` pages until exhausted (which turns one request into an unbounded number for a large org and needs its own loading-state design). Fourth consecutive Codex-review round to find something in this fetch call (GF-31 → GF-32 → this) — flagged as the stopping point rather than chased into a fifth variant, and low real-world severity: no department using this software is near 1,000 applications in a single status. Found by Codex review, security-review PR #2072, round 8; see `docs/security-review/GF-22-grants-fundraising.md` GF-33. (GF-32a)                                                                                                                                                                                                                  |
