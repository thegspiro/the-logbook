...
| **`PUT /training/skills-testing/tests/{id}` can set `status`/`result`/`overall_score` directly, bypassing `complete_test`** | Open (MED, security review SKT4-7, 2026-09-10)                                     | `SkillTestUpdate.status`/`.result`/`.overall_score` are unrestricted optional fields the handler assigns straight to the ORM row, and `_authorize_test_write` permits any member holding `examiner_id` on an unvalidated test to call it — not only `training.manage`. An examiner can therefore `PUT` a test straight to `status="completed"` with a hand-picked result/score and no `section_results` to justify them, skipping `complete_test`'s own computation entirely; the officer validation path then trusts and can credit that fabricated result. Closing it needs a decision about which fields a bare `PUT` may still set (in-progress fields) versus which must route through `complete_test` — a state-machine/API-contract decision, not a same-commit fix.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
