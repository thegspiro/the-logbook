...
| **`RateLimiter` tracks per-key state across three independent dicts (plus one per-scope dict) — recommend collapsing to one per-key record** | Open (design follow-up, 2026-09-07, security review CI3-33; recurrence confirmed 2026-09-07, PR #2369) | `self.requests`, `self.lockouts`, and `self._key_windows` (`security_middleware.py`) are three separately-capped, separately-evicted structures meant to describe the same key's state, and five successive Codex review rounds across two PRs (#2368, then #2369 after #2368 merged) each found a new way they — or a new mechanism added to patch a prior round — drift or misbehave: CI3-33-1/2 (forced eviction silently lifting an active lockout / undercounting a key's own history), CI3-33-1a (window metadata not restored alongside history), CI3-33-1b/1c (`self.lockouts` uncapped, then its by-expiry eviction itself releasing an unrelated key's active lockout), CI3-33-1d/1e (`self._key_windows` uncapped independent of the other two; a saturated-table violator's only remaining protection being the much shorter request-history window rather than its lockout duration), and CI3-33-2a/2b (round 5, PR #2369: the _new_ saturation-reject signal CI3-33-1e added was a single global scalar instead of being scoped per rate-limit scope — a cross-scope DoS — and a strict `>` eviction-gate comparison let a stale, already-expired lockout count trigger an unnecessary saturation rejection). All were fixed narrowly and are covered by regression tests, but the recurrence across five independent rounds — the last of which was a design gap in a mechanism built specifically to fix the round before it — is a strong signal the multi-structure shape is the root cause, not any one interaction. **Recommended follow-up:** collapse to a single dict keyed by `key`, holding `{requests: list[float], window_seconds: int, lockout_until: float \| None}`, plus keep the per-scope saturation signal as a separate small dict (its key space is fundamentally different — a fixed set of code-literal scopes, not attacker-influenceable identifiers) — one eviction/expiry path instead of several that can disagree, closing this defect class by construction rather than needing a sixth incremental patch. Not attempted in either PR: every fix has landed mid-incident against a live Codex round, and the `TestRateLimiter` suite (31 tests as of PR #2369) constructs its scenarios by writing directly into the internal dicts, so this is a deliberate, separately-reviewed and tested change, not a drive-by refactor. See `docs/security-review/CI3-33-core-infra.md` ("On not doing a structural refactor this pass" and its PR #2369 addendum). |
