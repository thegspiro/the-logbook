...
| **`RateLimiter` tracks per-key state across three independent dicts — recommend collapsing to one per-key record** | Open (design follow-up, 2026-09-07, security review CI3-33)                      | `self.requests`, `self.lockouts`, and `self._key_windows` (`security_middleware.py`) are three separately-capped, separately-evicted structures meant to describe the same key's state, and four successive Codex review rounds on the same PR (#2368) each found a new way they drift apart under partial-write/partial-evict interactions: CI3-33-1/2 (forced eviction silently lifting an active lockout / undercounting a key's own history), CI3-33-1a (window metadata not restored alongside history), CI3-33-1b/1c (`self.lockouts` uncapped, then its by-expiry eviction itself releasing an unrelated key's active lockout), and CI3-33-1d/1e (`self._key_windows` uncapped independent of the other two; a saturated-table violator's only remaining protection being the much shorter request-history window rather than its lockout duration). All were fixed narrowly and are covered by regression tests, but the recurrence across four independent rounds indicates the three-dict shape is the root cause, not any one interaction. **Recommended follow-up:** collapse to a single dict keyed by `key`, holding `{requests: list[float], window_seconds: int, lockout_until: float \| None}` — one eviction/expiry path instead of three that can disagree, closing this defect class by construction rather than needing a fifth incremental patch. Not attempted in the CI3-33 pass itself: every fix there landed mid-incident against a live Codex round, and the existing 29-test `TestRateLimiter` suite constructs its scenarios by writing directly into the three current dicts, so this is a deliberate, separately-reviewed and tested change, not a drive-by refactor. See `docs/security-review/CI3-33-core-infra.md` ("On not doing a structural refactor this pass"). |
