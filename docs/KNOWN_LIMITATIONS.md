...
| **Build: the linter's TypeScript is an npm-auto-installed peer, not a declaration**            | Open (LOW-MED, build hygiene)                                          | Dependabot bumped `typescript` 5.9.3 → 7.0.2 in `frontend/package.json` on 2026-08-17, so `typescript` and the `typescript-native` alias now name the same version and the alias is redundant. `typescript-eslint@8.67.0` still caps its peer at `>=4.8.4 <6.1.0`, and type-aware lint runs against a **5.9.3 that no manifest in this repository asks for** — `package-lock.json` carries `node_modules/typescript@5.9.3` marked `"peer": true`, auto-installed by npm to satisfy that range. CI is green, so nothing is broken today; the risk is that the pin went from **declared** to **incidental**, and a different npm version, a `--strict-peer-deps` install, or a lockfile regeneration is not guaranteed to reproduce it. Two honest fixes: re-pin `typescript` at `5.9.3` (restoring the declared split documented in CLAUDE.md), or keep 7.0.2 and pin the linter's compiler another way. Do **not** delete the alias without deciding which. (BUILD-1) |
