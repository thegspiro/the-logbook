...
| **Events: public event-request intake has no per-org opt-in or spam parity with forms**          | Open (MED, feature + config) — **gap widened, see below**              | `POST /event-requests/public?organization_id=…` accepts a submission from anyone who supplies an active org's id (discoverable via the public calendar). Its only gate is a per-IP limit of 10; each submission writes rows and fires assignee/requester emails. There is no per-org "accept public requests" toggle, no honeypot, and no per-org daily cap. **The parity gap is now wider, not narrower** _(2026-08-17)_: the public _forms_ submit route gained `require_captcha` alongside its honeypot and valid-only daily cap, and password reset gained a challenge too — event-request intake was not included, leaving it the remaining unchallenged internet-exposed write path. Closing it is a build-out whose shape depends on a product call: public intake **off by default** for every organization (safe, but silently breaks any department relying on it today) versus **on with a challenge plus a per-org daily cap** (keeps the workflow, adds a dependency on the captcha provider being reachable). (EV-5)                                               |
