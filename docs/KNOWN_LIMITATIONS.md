...
| **`GET /api/public/v1/events/public` answered 500 whenever an events field was whitelisted — resolved by matching the model to the handler** | ✅ Resolved (2026-09-24, security review PUB-7, PR #2665)                        | `PublicEvent` declared all eight of its fields as required, and three of the names — `start_time`, `end_time`, `is_public` — were not keys the handler produced, so any non-empty filtered event raised `ValidationError` and the route could answer 200 only by returning `[]`. The decision the flag was waiting on: the **handler's** names win, because `public_portal_data_whitelist` rows are keyed by `field_name` and re-keying the handler to the documented `start_time`/`end_time` would orphan every row an administrator had enabled — including the ones this same PR's field catalogue now creates for every organization. `docs/PUBLIC_API_DOCUMENTATION.md` documented a response that had never been served (no client can have been written against a shape that 500s), so it was corrected rather than preserved. `id` became whitelistable rather than always-on, keeping "only whitelisted fields are returned" true; `is_public` was dropped, as `Event` has no such column and the query already restricts the list to non-cancelled, non-draft, future `public_education` events. `tests/test_public_portal_whitelist_shape.py` now pins `PublicEvent`'s field names to the field catalogue, which `tests/test_public_portal_whitelist_catalogue.py` pins to the handler's own dictionary.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| **`POST`/`PUT /training/skills-testing/tests(/{id})` result arrays and viewer-position lists have no per-field item cap**   | Open (MED, security review SKT4-2, 2026-09-10)                                     | `SkillTestUpdate.section_results`, each `SectionResultSchema.criteria_results`, and `CriterionResultSchema.checklist_completed` (`app/schemas/skills_testing.py`) have no `max_length` — the same gap as SKT4-1, one level deeper. Unlike SKT4-1, `update_test` is gated by `_authorize_test_write`, which any member holding the `examiner_id` role on an unvalidated test can satisfy, not `training.manage` alone — so the reachable population is every member, not just officers. `SkillTestCreate`/`SkillTestUpdate.result_viewer_positions` (`Optional[List[str]]`, no `max_length`) is a third such field, and `_guard_official_test_policy_fields` deliberately lets an ordinary member set it on their own self-practice test at creation time too, not just on update. Closing it needs `max_length` values sized to the largest legitimate in-progress test, not just the largest template — a content decision, not a same-commit fix.                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| **`PUT /training/skills-testing/tests/{id}` can set `status`/`result`/`overall_score` directly, bypassing `complete_test`** | Open (MED, security review SKT4-7, 2026-09-10, widened SKT5, 2026-09-15)           | `SkillTestUpdate.status`/`.result`/`.overall_score` are unrestricted optional fields the handler assigns straight to the ORM row, and `_authorize_test_write` permits any member holding `examiner_id` on an unvalidated test to call it — not only `training.manage`. An examiner can therefore `PUT` a test straight to `status="completed"` with a hand-picked result/score and no `section_results` to justify them, skipping `complete_test`'s own computation entirely; the officer validation path then trusts and can credit that fabricated result. **Since a 2026-09-12 feature commit** (`unresolved_criteria`/`waived_critical_criteria`, `app/services/skills_testing_service.py`), `complete_test` also refuses a scorecard with a blank non-statement step or a waived critical step — but that check lives only in `complete_test`, so the same bare-`PUT` bypass now also skips the guard specifically built to stop a silently-wrong result from being filed. No shipped frontend caller exercises the bypass today (`ActiveSkillTestPage.tsx`'s seven `updateTest`/`saveTest` call sites never send `status: "completed"`). Closing it needs a decision about which fields a bare `PUT` may still set (in-progress fields) versus which must route through `complete_test` — a state-machine/API-contract decision, not a same-commit fix. |
...
| **Finance: eight non-ledger status transitions still read their entity via a plain SELECT**                                                                                  | 🚩 Open (LOW, 2026-09-14, security-review FIN-05 pass 5 — needs its own concurrency-test pass)                                | FIN-31 (pass 5) locked the five status transitions that mutate the shared `Budget` ledger — `mark_pr_paid`, `cancel_purchase_request`, `mark_expense_paid`, `issue_check`, `void_check` — after finding a concrete double-credit/status-flip-flop race (CLAUDE.md Pitfall #27, the same shape FIN-10 fixed for `approve_step`/`deny_step`). The same unlocked-read-then-write shape remains on `submit_purchase_request`/`submit_expense_report`/`submit_check_request` (a race could create two parallel sets of `ApprovalStepRecord`s for one entity, since both racers read `status=DRAFT` before either commits and both call `create_approval_records`) and on `mark_pr_ordered`/`mark_pr_received`/`update_purchase_request`/`update_expense_report`/`update_check_request` (non-terminal, non-ledger transitions — a raced double-submit here is a duplicate-state defect, not silent money-accounting corruption). Lower severity than FIN-31 for exactly that reason: none of these eight touches `Budget`. **Why not fixed in the pass that found it:** the mechanical fix is the same one-line-per-method pattern FIN-31 used (an inline `select(...).with_for_update()` in place of the plain getter call), but applying it uniformly across eight more methods in the same commit — without a dedicated concurrency test for each interaction (submit-vs-submit, update-vs-submit, mark-ordered-vs-cancel) — risks a rushed, under-tested sweep across a financial/approvals-critical surface. Fix pattern for whoever picks this up: mirror `mark_pr_paid`'s locked read exactly (`backend/app/services/finance_service.py`), and add a `TestFinanceDisbursementLocking`-style source-inspection guard test per method in `tests/test_capacity_locking.py`. See [`docs/security-review/FIN-05-finance-approvals.md`](./security-review/FIN-05-finance-approvals.md) → Pass 5, FIN-31.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| **Inventory: five medical-domain mutation paths validate domain membership before the lock, not under it**                           | Open (LOW-MED, needs a per-method locking-domain-recheck design)                                                                                                                        | `update_medical_item`, `add_medical_item_lot`, `receive_medical_delivery`, `update_medical_lot`, and `delete_medical_lot` all validate domain membership via a plain-read preflight (`_require_medical_item`/`_require_medical_category`) before delegating to the underlying `InventoryService` mutation — the same shape `retire_medical_item` had before it was closed (MSUP-20/21) by re-validating domain membership under the item's row lock once acquired. If a broad `inventory.manage` caller reclassifies the item or its category between the preflight and the mutation's own lock (where one exists), a medical-only caller (`inventory.manage_medical` only) could mutate a row that raced out of their domain in that narrow window. A second, independently-discovered instance of the same systemic shape: the maintenance-completion path (`complete_maintenance` / `InventoryMaintenancePage.tsx`'s "Condition After Work" picker) can write a RETIRED condition entirely outside `retire_item`'s locking, blocker checks, and audit trail, leaving `active` untouched. Lower severity than retirement itself — these are same-organization, non-destructive field edits (rename, quantity, lot dates), not an irreversible removal from active inventory, and the race requires a reclassification to land in the same narrow window as an edit. Closing it needs one shared "validate domain under this mutation's own lock" contract designed against each method's own transaction shape (`update_item`'s `needs_lock` branching, `add_lot`'s two-phase locked carry-forward, `add_lots_bulk`'s per-line loop, `update_lot`/`delete_lot` which may not lock the parent item at all), not a single mechanical patch applied ad hoc to five call sites. Found by Codex review, security-review PR #2301 (MSUP-23 pass 9 and pass 10); see `docs/security-review/MSUP-23-medical-supplies.md` MSUP-25. (MSUP-25)                  |
...
| Deactivate a leave                   | `DELETE /users/leaves-of-absence/{id}`                                                                                  | `deleteLeaveOfAbsence`                                                                    | `WaiverManagementPage.tsx` (**Deactivate**)                                                                             | ✅ Shipped                           |
| List archived / reactivate           | `GET /users`, `POST /users/{id}/reactivate`                                                                             | `getUsers`, `reactivateMember`                                                            | `Members.tsx` (Archived filter + Reactivate row action), `MemberProfilePage.tsx` (status control on an archived member) | ✅ Shipped (2026-09-24)              |
...
| Tier configuration                   | `GET`/`PUT /users/membership-tiers/config`, `POST /users/advance-membership-tiers`, `PATCH /users/{id}/membership-type` | `getTierConfig`, `updateTierConfig`, `advanceMembershipTiers`                             | `TiersSection.tsx` (Members Admin → Settings → Membership Tiers) and onboarding; `advanceMembershipTiers` has no caller | ✅ Shipped (manual advance API only) |
...
| Item                                                        | Status                            | Detail                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ----------------------------------------------------------- | --------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **No UI to un-archive a member**                            | ✅ Resolved (2026-09-24)          | The Members list offers an **Archived** status filter to `members.manage` holders, with a **Reactivate** action on each archived row; an archived member's profile turns its status control into the same Reactivate dialog. Correction to the earlier wording: archiving was never available from the profile either — the status dialog offered `Archived` but the backend rejects it (`/archive` is a dedicated endpoint), so the option has been removed. Archiving still happens automatically once a dropped member has returned everything, or through `POST /users/{id}/archive`; a manual Archive button is not built. `GET /users/archived` / `getArchivedMembers` remain uncalled, because the roster already returns archived members. |
...
| **No UI for membership tier configuration**                 | ✅ Resolved                       | Members Admin → Settings → Membership Tiers edits the ladder, benefits and auto-advance switch (also an onboarding step). Corrected 2026-09-24 during a membership-guide docs pass; this row had stayed open after the screen shipped.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
