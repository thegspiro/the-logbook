...
| **`RateLimiter` tracks per-key state across three independent dicts (plus one per-scope dict) — collapse to one per-key record: now the recommended next priority on this file, not an indefinitely-deferred item** | 🚩 Open (design follow-up — RECOMMENDED NEXT PRIORITY; 2026-09-07, security review CI3-33; recurrence confirmed 2026-09-07 PR #2370 round 5; round-6 trigger met 2026-09-07 PR #2370 round 6) | `self.requests`, `self.lockouts`, and `self._key_windows` (`security_middleware.py`) are three separately-capped, separately-evicted structures meant to describe the same key's state, and six successive Codex review rounds across two PRs (#2368, then #2370 — the latter covering two Codex rounds, since #2370's own first-round fix drew a second round of review before it could land) each found a new way they — or a new mechanism added to patch a prior round — drift or misbehave: CI3-33-1/2 (forced eviction silently lifting an active lockout / undercounting a key's own history), CI3-33-1a (window metadata not restored alongside history), CI3-33-1b/1c (`self.lockouts` uncapped, then its by-expiry eviction itself releasing an unrelated key's active lockout), CI3-33-1d/1e (`self._key_windows` uncapped independent of the other two; a saturated-table violator's only remaining protection being the much shorter request-history window rather than its lockout duration), CI3-33-2a/2b (round 5: the _new_ saturation-reject signal CI3-33-1e added was a single global scalar instead of being scoped per rate-limit scope — a cross-scope DoS — and a strict `>` eviction-gate comparison let a stale, already-expired lockout count trigger an unnecessary saturation rejection), and **CI3-33-2c (round 6: CI3-33-2b's own fix — forcing an immediate sweep once `self.lockouts` merely reached capacity — turned every request sharing this limiter into a full three-dict scan for as long as an attacker kept the table full, a CPU-amplification DoS)**. All six were fixed narrowly and are covered by regression tests, but round 6 is a fix generating the next round's finding, in the exact same code path a prior round already touched — a materially stronger signal than another dict merely disagreeing with another dict. **This row previously said a sixth round of this general shape would be treated as a strong signal to stop patching; that round has now happened, and the call is made here rather than deferred again: the refactor should be the next piece of work on this file, before a seventh round, not an indefinitely-deferred design item.** Recommended shape: collapse to a single dict keyed by `key`, holding `{requests: list[float], window_seconds: int, lockout_until: float \| None}`, plus keep the per-scope saturation signal as a separate small dict (its key space is fundamentally different — a fixed set of code-literal scopes, not attacker-influenceable identifiers) — one eviction/expiry path instead of several that can disagree, closing this defect class by construction. Not attempted in either PR so far: every fix has landed mid-incident against a live Codex round, and the `TestRateLimiter` suite (33 tests as of PR #2370) constructs its scenarios by writing directly into the internal dicts, so this remains a deliberate, separately-reviewed and tested change, not a drive-by refactor folded into a bug-fix commit — the reasoning for keeping the fix itself narrow is unchanged even though the recommendation for what happens next is not. See `docs/security-review/CI3-33-core-infra.md` (CI3-33-2c's "On the structural refactor" section). |
