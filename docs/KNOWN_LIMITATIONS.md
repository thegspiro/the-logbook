...
| **Security monitoring: alert/intrusion-detection endpoints have no admin UI**                                 | 🚩 Open — needs a frontend build                                                                                              | None of `security_monitoring.py`'s 13 endpoints (`/security/status`, `/alerts`, `/alerts/{id}/acknowledge`, `/alerts/{id}/resolve`, `/audit-log/integrity`\|`/status`\|`/checkpoint`\|`/rehash`\|`/entries`\|`/export`, `/intrusion-detection/status`, `/data-exfiltration/status`, `/manual-check`) has a working frontend consumer — `securityService` in `frontend/src/services/adminServices.ts` wraps five of them but is called from nowhere in the app (confirmed by exhaustive grep). This is real detective capability going unseen, not dead scaffolding: brute-force (`endpoints/auth.py`), session-hijack + data-exfiltration (`core/security_middleware.py`), and privilege-escalation (`users.py`/`roles.py`) detection are all wired in and create `ThreatLevel.CRITICAL` `security_alerts` rows on the exact attack patterns these detectors exist for — none of which any admin can currently see, acknowledge, or resolve through the UI; the only way is a direct DB query or a raw API call. By contrast `AuditLogPage`, `ErrorMonitoringPage`, and `IPSecurityAdminPage` (this feature's other three backend files) all have working, permission-gated admin screens — this gap is specific to `security_monitoring.py`. Closing it is a genuine feature build (route, permission-gate decision, alert list/detail view, ack/resolve actions), not a drive-by fix. Found in security-review SEC2-28 pass 2; see `docs/security-review/SEC2-28-security-audit-ip.md`.                                                                               |
