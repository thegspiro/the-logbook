...
**Goal.** Let an administrator export a department's _configuration and
definitions_ — training requirements, inventory categories/allowances, meeting
...
- Members/users, positions _assignments_, prospects, sessions, password history.
...
- Transactional data & history: actual meetings, events, elections/votes,
  ...
  logs.
- Uploaded files: documents, photos, attachments (the folder _structure_ is
...
> Because members are excluded, this is **not** a disaster-recovery _backup_ of
...
1. **Map phase.** For every exported row, compute its _target_ id:
   ...
   - all others → mint a new UUID.
     Result: a complete `old_id → new_id` map across all tables _before_ any
     writes.
...
Building the _entire_ id map up front (phase 1) is what lets JSON arrays that
...
| `training_courses`    | —                  | JSON `prerequisites`, `category_ids` remap; FK `program_id` remap _(2026-08-05)_                                                                                                                                                                                                         |
| `course_classes`      | parent `course_id` | _(2026-08-05)_ **Include with the course** — a multi-class course is not portable without its syllabus. FKs `class_course_id` (required), `instructor_id`, `location_id`, `category_id`, `requirement_id`, `phase_id` remap; **scrub `instructor_id`** (a user of the source department) |
...
| `course_cohorts`, `course_cohort_classes`, `course_cohort_members` | — | _(2026-08-05)_ **Excluded — operational, not template.** A cohort is one department's dated run of a course, carrying real event/session ids, a roster of real members, and attendance. A template ships the _syllabus_; the receiving department generates its own cohorts from it |
...
**Referential-completeness guard.** If the admin exports a _subset_ of modules,
...
- **S3** (stored XSS / SSRF from imported content) — a super user's session is a
  _higher-value_ XSS target, not a lower one.
...
- Tests: round-trip parity + id/JSON-ref integrity; **no secret/user/PII
  leakage** (column _and_ JSON scan); FK-coverage guard; **abuse tests** —
