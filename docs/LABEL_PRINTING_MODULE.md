...
| Inventory label content                                     | `extra_lines` keys `location`, `storage_area`, `category`, `size`, `condition` add one line, in order; `no_name` / `no_asset_tag` / `no_serial_number` drop a line that prints by default (`LabelSpec.show_name`, honoured by the PDF, ZPL and ESC/POS renderers). The inventory preset saves them (omitted keeps the saved choice) |
