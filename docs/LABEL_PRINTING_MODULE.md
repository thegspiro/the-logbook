...
| Inventory label content                                     | `extra_lines` keys `location`, `storage_area`, `category`, `size`, `condition` add one line, in order; `no_asset_tag` / `no_serial_number` drop an identifier line that prints by default. The inventory preset saves them (omitted keeps the saved choice) |
