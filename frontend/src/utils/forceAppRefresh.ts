 ...
 *
 * Order matters. The caches are dropped *before* the service worker swap so the
 * incoming worker precaches the new build rather than having its fresh precache
 * deleted out from under it. The reload happens unconditionally — if any step
 * fails the user still gets a reload, which is what they asked for.
 */
