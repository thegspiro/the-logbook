...
export const GrantOpportunitiesPage: React.FC = () => {
  ...
  const filteredOpportunities = useMemo(() => {
    ...
    if (search) {
      ...
      list = list.filter((o) => o.name.toLowerCase().includes(q) || o.agency.toLowerCase().includes(q));
    }
    if (categoryFilter) {
    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="mb-3 flex items-center gap-2">
          <Landmark className="text-theme-text-muted h-5 w-5" />
          <h2 className="text-theme-text-primary text-lg font-semibold">Federal Programs</h2>
        </div>
            ...
            <div key={program.code} className="border-theme-surface-border bg-theme-surface rounded-lg border p-4">
              <div className="mb-2 flex items-start justify-between">
                <div>
                  ...
                  <p className="text-theme-text-secondary text-xs">{program.agency}</p>
                </div>
                <span
              ...
              <p className="text-theme-text-secondary text-sm">{program.description}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Opportunities Grid */}
      <div>
        <h2 className="text-theme-text-primary mb-3 text-lg font-semibold">All Opportunities</h2>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12">
            <BookOpen className="text-theme-text-secondary mb-3 h-12 w-12 opacity-40" />
            <p className="text-theme-text-secondary">No grant opportunities found</p>
          </div>
        ) : (
              ...
              return (
                <div key={opp.id} className="border-theme-surface-border bg-theme-surface rounded-lg border p-4">
                  ...
                  <div className="mb-2 flex items-center gap-2 text-sm">
                    ...
                    {urgency.isUrgent && (
                      <span className="inline-flex items-center gap-1 rounded bg-red-100 px-1.5 py-0.5 text-xs font-medium text-red-700 dark:bg-red-500/20 dark:text-red-400">
                  ...
                  {opp.matchRequired && (
                    <div className="mb-3 flex items-center gap-2 text-sm">
                      ...
                      <span className="text-amber-700 dark:text-amber-400">
                        Match required
                        {opp.matchPercentage != null ? ` (${opp.matchPercentage}%)` : ''}
                      </span>
                    </div>
                  )}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="bg-theme-surface-hover text-theme-text-secondary hover:text-theme-text-primary inline-flex items-center gap-1 rounded-lg px-3 py-1.5 text-sm font-medium transition-colors"
                    >
                      {isExpanded ? (
                        <>
                  ...
                  {/* Expanded Details */}
                  {isExpanded && (
                    <div className="border-theme-surface-border mt-3 space-y-2 border-t pt-3 text-sm">
                      {opp.description && (
                        <div>
                          <p className="text-theme-text-muted mb-1 text-xs font-medium uppercase">Description</p>
                          <p className="text-theme-text-secondary">{opp.description}</p>
                        </div>
                      )}
                      {opp.eligibleUses && (
                        <div>
                          <p className="text-theme-text-muted mb-1 text-xs font-medium uppercase">Eligible Uses</p>
                          <p className="text-theme-text-secondary">{opp.eligibleUses}</p>
                        </div>
                      )}
                      {opp.eligibilityCriteria && (
                        <div>
                          <p className="text-theme-text-muted mb-1 text-xs font-medium uppercase">
                            Eligibility Criteria
                          </p>
                          <p className="text-theme-text-secondary">{opp.eligibilityCriteria}</p>
                        </div>
                      )}
                      {opp.matchDescription && (
                        <div>
                          <p className="text-theme-text-muted mb-1 text-xs font-medium uppercase">Match Details</p>
