...
import {
  ...
  XCircle, MinusCircle, Calendar, ArrowLeft, AlertTriangle, Pencil,
} from 'lucide-react';
...
export default function InspectionsListPage() {
  ...
  const openCreate = () => {
    ...
    setFormData({
      facility_id: facilityFilter || '', inspection_type: 'routine', title: '',
      description: '', inspection_date: getTodayLocalDate(tz), next_inspection_date: '',
      inspector_name: '', inspector_organization: '', passed: '', findings: '',
      corrective_actions: '', corrective_action_deadline: '', notes: '',
    });
    setShowModal(true);
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-theme-text-primary">Inspections</h1>
            <p className="text-theme-text-secondary text-sm mt-0.5">Track and manage inspections across all facilities</p>
          </div>
        </div>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="relative flex-1 max-w-md">
          ...
          <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
            aria-label="Search inspections..." placeholder="Search inspections..." className="form-input pl-10 placeholder-theme-text-muted pr-4 py-2.5" />
        </div>
        <select value={facilityFilter} onChange={e => setFacilityFilter(e.target.value)} className={inputCls + ' max-w-xs'}>
          ...
          <div className="bg-theme-surface-modal border border-theme-surface-border rounded-xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div><label className={labelCls}>Next Inspection</label><input type="date" value={formData.next_inspection_date} onChange={e => setFormData(p => ({...p, next_inspection_date: e.target.value}))} className={inputCls} /></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className={labelCls}>Inspector</label><input type="text" value={formData.inspector_name} onChange={e => setFormData(p => ({...p, inspector_name: e.target.value}))} className={inputCls} /></div>
