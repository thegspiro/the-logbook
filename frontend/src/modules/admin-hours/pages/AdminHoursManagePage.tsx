...
import { Plus, Pencil, Trash2, QrCode, Check, X, Clock, Download, ChevronLeft, ChevronRight, AlertTriangle, Timer, StopCircle, RefreshCw } from 'lucide-react';
import { useAdminHoursStore } from '../store/adminHoursStore';
import { adminHoursEntryService } from '../services/api';
import type { AdminHoursCategoryCreate, AdminHoursCategoryUpdate, AdminHoursCategory, AdminHoursEntryEdit } from '../types';
...
const AdminHoursManagePage: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'categories' | 'active' | 'pending' | 'all' | 'summary'>('categories');
  ...
  }, [activeTab, fetchCategories, fetchAllEntries, fetchActiveSessions, fetchSummary, fetchPendingCount, allStatusFilter, allCategoryFilter, allPage, pendingPage]);
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      ...
      <div className="flex gap-1 mb-6 bg-theme-surface rounded-lg p-1">
        {(['categories', 'active', 'pending', 'all', 'summary'] as const).map((tab) => (
          <button
            key={tab}
          ...
          >
            {tab === 'categories' ? 'Categories' : tab === 'active' ? 'Active Sessions' : tab === 'pending' ? 'Pending Review' : tab === 'all' ? 'All Entries' : 'Summary'}
