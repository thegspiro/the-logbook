 ...
 * loading, the one-time localStorage migration, and the offline fallback
 * chain (cache → localStorage mirror → built-in defaults).
 */
...
describe('loadShiftSettings', () => {
  it('returns backend settings and mirrors them to localStorage when stored', async () => {
    ...
    const mirror = JSON.parse(localStorage.getItem(SETTINGS_KEY) ?? '{}') as ShiftSettings;
    expect(mirror.defaultDurationHours).toBe(24);
  });
