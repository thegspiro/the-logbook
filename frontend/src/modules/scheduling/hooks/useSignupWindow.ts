 ...
 *
 * Returns `UNRESOLVED_SIGNUP_WINDOW` until the settings land, so a board
 * painted before the fetch completes does not disable a button the server
 * would have accepted. That matters for longer than one paint: a failed
 * `loadSettings` deliberately leaves the settings unloaded, and nothing
 * retries until the next mount, so this is the window the screen keeps.
 */

import { useEffect, useMemo, useSyncExternalStore } from 'react';
import { useSchedulingStore } from '../store/schedulingStore';
import { UNRESOLVED_SIGNUP_WINDOW, type SignupWindow } from '../utils/shiftBoard';
