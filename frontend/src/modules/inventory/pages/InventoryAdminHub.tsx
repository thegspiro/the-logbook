          ...
          <div className="mb-8 bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-4">
            ...
            <div className="space-y-1">
              {lowStockAlerts.slice(0, 5).map((alert) => (
                <p key={alert.category_id} className="text-xs text-yellow-600 dark:text-yellow-400">
                  {alert.category_name}: {alert.current_stock} items (threshold: {alert.threshold})
                </p>
              ))}
              {lowStockAlerts.length > 5 && (
                <p className="text-xs text-yellow-500">...and {lowStockAlerts.length - 5} more</p>
              )}
            </div>
          </div>
        )}
