...
describe('PullToRefreshIndicator', () => {
  it('renders nothing when idle', () => {
    render(<PullToRefreshIndicator pulling={false} refreshing={false} pullDistance={0} />);
  ...
  it('renders when pulling', () => {
    render(<PullToRefreshIndicator pulling={true} refreshing={false} pullDistance={40} />);
    expect(screen.getByLabelText('Pull to refresh')).toBeInTheDocument();
  });

  it('shows refreshing label when refreshing', () => {
    render(<PullToRefreshIndicator pulling={false} refreshing={true} pullDistance={0} />);
    expect(screen.getByLabelText('Refreshing')).toBeInTheDocument();
  });

  it('caps arrow rotation at progress=1', () => {
    render(<PullToRefreshIndicator pulling={true} refreshing={false} pullDistance={80} threshold={80} />);
