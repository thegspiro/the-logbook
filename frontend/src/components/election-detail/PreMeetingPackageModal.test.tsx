...
vi.mock('../../services/api', () => ({
  electionService: {
    getPackageRecipients: (...args: unknown[]) => mockGetPackageRecipients(...args) as unknown,
    downloadPackagePdf: (...args: unknown[]) => mockDownloadPackagePdf(...args) as unknown,
  },
}));
...
describe('PreMeetingPackageModal', () => {
  ...
  it('prefills from leadership and defaults to the full roster variant', async () => {
    ...
    expect(screen.getByRole('checkbox', { name: /full roster detail/i })).toBeChecked();
  });
  ...
  it('prefills from eligible voters and defaults to the member variant', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'All eligible voters' }));

    await waitFor(() => {
      expect(mockGetPackageRecipients).toHaveBeenCalledWith('el1', 'eligible_voters');
    });
    expect(screen.getByRole('checkbox', { name: /full roster detail/i })).not.toBeChecked();
  });
  ...
  it('supports removing prefilled recipients and adding free-form addresses', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Remove pat@dept.org' }));
    expect(screen.queryByText(/pat@dept\.org/)).not.toBeInTheDocument();

    await user.type(screen.getByLabelText('Add an email address'), 'counsel@lawfirm.example');
    await user.click(screen.getByRole('button', { name: 'Add' }));

    await user.click(screen.getByRole('button', { name: /Send to 2 recipients/ }));

    expect(onSubmit).toHaveBeenCalledWith(['sue@dept.org', 'counsel@lawfirm.example'], '', true);
  });
  ...
  it('rejects malformed free-form addresses', async () => {
    ...
    await user.type(screen.getByLabelText('Add an email address'), 'not-an-email');
    await user.click(screen.getByRole('button', { name: 'Add' }));
  ...
  it('shows the send error and closes on cancel', async () => {
    const user = userEvent.setup();
    render(<PreMeetingPackageModal {...defaultProps} error="SMTP unavailable" />);
