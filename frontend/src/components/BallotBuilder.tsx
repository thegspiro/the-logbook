    ...
    <div className="bg-white/10 backdrop-blur-sm rounded-lg p-6">
      ...
      {showTemplates && !selectedTemplate && (
        <div className="mb-6 p-4 bg-indigo-500/10 rounded-lg border border-indigo-500/30">
          <h4 className="text-sm font-semibold text-indigo-300 mb-3">Select a Template</h4>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {templates.map((template) => (
              <button
                ...
                className="text-left p-3 bg-white/5 rounded-lg border border-indigo-500/30 hover:border-indigo-400 hover:bg-white/10 transition-all"
              >
                <div className="font-medium text-white text-sm">{template.name}</div>
                <p className="text-xs text-slate-400 mt-1">{template.description}</p>
                <div className="flex gap-2 mt-2">
                  <span className="px-2 py-0.5 text-xs bg-indigo-500/20 text-indigo-300 rounded">
          ...
          <div className="space-y-3">
            <div>
              ...
              <input
                ...
                className="mt-1 block w-full bg-slate-900/50 border border-slate-600 rounded-md shadow-sm py-2 px-3 text-white focus:ring-indigo-500 focus:border-indigo-500 text-sm"
                placeholder={
                  selectedTemplate.type === 'membership_approval'
                    ? 'e.g., John Smith'
                    : selectedTemplate.type === 'officer_election'
      ...
      {ballotItems.length === 0 ? (
        ...
        <div className="space-y-3">
          {ballotItems.map((item, index) => (
            <div
              key={item.id}
              className="p-4 rounded-lg border border-white/20 bg-white/5 hover:bg-white/10 transition-shadow"
            >
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  {item.description && (
                    <p className="mt-1 ml-8 text-sm text-slate-400">{item.description}</p>
                  )}
