...
const PipelineSettingsPage: React.FC = () => {
  ...
  const getStepTypeIcon = (type: string) => {
    switch (type) {
      case 'action': return <Zap className="h-4 w-4 text-blue-700 dark:text-blue-400" />;
      case 'note': return <MessageSquare className="h-4 w-4 text-yellow-700 dark:text-yellow-400" />;
      default: return <CheckSquare className="h-4 w-4 text-green-700 dark:text-green-400" />;
    }
  };
  ...
  return (
    <div className="min-h-screen space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <Link to="/membership-pipeline" className="flex items-center gap-2 text-theme-text-muted hover:text-theme-text-primary transition-colors mb-2">
          ...
          <h1 className="text-2xl font-bold text-theme-text-primary flex items-center gap-3">
            <Settings className="h-7 w-7 text-red-700 dark:text-red-500" />
            Pipeline Settings
          </h1>
      ...
      {successMsg && (
        <div className="bg-green-900/50 border border-green-700 text-green-700 dark:text-green-300 rounded-lg p-3 text-sm">
          {successMsg}
        </div>
      )}
      {error && (
        <div className="bg-red-900/50 border border-red-700 text-red-700 dark:text-red-300 rounded-lg p-4 flex items-center justify-between">
          <span>{error}</span>
          <button onClick={() => setError(null)} className="text-red-700 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300"><X className="h-4 w-4" /></button>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          ...
          {templates.length > 0 && (
            <div>
              <h3 className="text-sm font-medium text-theme-text-muted uppercase tracking-wider mb-2">Templates</h3>
                  ...
                  <div
                    ...
                    className={`p-4 rounded-xl border cursor-pointer transition-colors ${
                      selectedPipeline?.id === p.id
                        ...
                        : 'bg-slate-800/50 border-theme-surface-border hover:border-theme-input-border'
                    }`}
                  >
                    <div className="flex items-center justify-between">
                      <span className="font-medium text-theme-text-primary text-sm">{p.name}</span>
                      <button
                    ...
                    <div className="flex items-center gap-3 mt-1 text-xs text-theme-text-muted">
                      ...
                      <span className="bg-yellow-500/20 text-yellow-700 dark:text-yellow-400 px-2 py-0.5 rounded-full">template</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {/* Custom Pipelines */}
          <div>
            <h3 className="text-sm font-medium text-theme-text-muted uppercase tracking-wider mb-2">Your Pipelines</h3>
            {customPipelines.length === 0 ? (
              <div className="text-center py-8 bg-slate-800/50 rounded-xl border border-theme-surface-border">
                <FileText className="h-8 w-8 mx-auto text-slate-500 mb-2" />
                <p className="text-sm text-theme-text-muted">No custom pipelines yet</p>
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        {p.is_default && <Star className="h-4 w-4 text-yellow-700 dark:text-yellow-500" />}
                        <span className="font-medium text-theme-text-primary text-sm">{p.name}</span>
                      </div>
                      <button
                        onClick={e => { e.stopPropagation(); handleDeletePipeline(p.id); }}
                        className="p-1 text-theme-text-muted hover:text-red-700 dark:hover:text-red-400 transition-colors"
                    ...
                    <div className="flex items-center gap-3 mt-1 text-xs text-theme-text-muted">
                      ...
                      {p.auto_transfer_on_approval && (
                        <span className="bg-green-500/20 text-green-700 dark:text-green-400 px-2 py-0.5 rounded-full">auto-transfer</span>
                      )}
                    </div>
                  </div>
                ))}
        ...
        <div className="lg:col-span-2">
          {!selectedPipeline ? (
            <div className="text-center py-16 bg-slate-800/50 rounded-xl border border-theme-surface-border">
              ...
              <p className="text-theme-text-muted">Select a pipeline to configure its steps</p>
            </div>
          ) : (
            <div className="space-y-6">
              {/* Pipeline Settings */}
              <div className="bg-slate-800/50 rounded-xl border border-theme-surface-border p-6">
                <h2 className="text-lg font-semibold text-theme-text-primary mb-4">{selectedPipeline.name}</h2>
                {selectedPipeline.description && (
                  <p className="text-sm text-theme-text-muted mb-4">{selectedPipeline.description}</p>
                )}
                <div className="flex flex-wrap gap-4">
                  <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    />
                    <span className="text-sm text-theme-text-secondary">Default pipeline for new prospects</span>
                  </label>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="checkbox"
                      ...
                      className="rounded border-theme-input-border bg-slate-700 text-red-700 dark:text-red-500 focus:ring-red-500"
                    />
                    <span className="text-sm text-theme-text-secondary">Auto-transfer to membership on final step completion</span>
                  </label>
                </div>
              </div>
              ...
              <div className="bg-slate-800/50 rounded-xl border border-theme-surface-border p-6">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-lg font-semibold text-theme-text-primary">Pipeline Steps</h3>
                  ...
                  <div className="space-y-2">
                    {selectedPipeline.steps
                      ...
                      .map((step, idx) => (
                        <div
                          key={step.id}
                          className="flex items-center gap-3 p-3 bg-slate-700/30 rounded-lg border border-theme-input-border/50 group"
                        >
                          {/* Reorder buttons */}
                          {!selectedPipeline.is_template && (
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              <span className="font-medium text-theme-text-primary text-sm">{step.name}</span>
                              {step.is_first_step && (
                            ...
                            <div className="flex items-center gap-2 mt-0.5">
                              <span className={`text-xs px-2 py-0.5 rounded-full ${
                                step.step_type === 'action' ? 'bg-blue-500/20 text-blue-700 dark:text-blue-400' :
                                step.step_type === 'note' ? 'bg-yellow-500/20 text-yellow-700 dark:text-yellow-400' :
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-slate-800 rounded-xl border border-theme-surface-border w-full max-w-lg">
            ...
            <form onSubmit={handleCreatePipeline} className="p-6 space-y-4">
              {createError && (
                <div className="bg-red-900/50 border border-red-700 text-red-700 dark:text-red-300 rounded-lg p-3 text-sm">{createError}</div>
              )}
              <div>
              ...
              <div className="space-y-2">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    className="rounded border-theme-input-border bg-slate-700 text-red-700 dark:text-red-500 focus:ring-red-500"
                  />
                  <span className="text-sm text-theme-text-secondary">Set as default pipeline</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
                    ...
                    className="rounded border-theme-input-border bg-slate-700 text-red-700 dark:text-red-500 focus:ring-red-500"
                  />
                  <span className="text-sm text-theme-text-secondary">Auto-transfer on final step completion</span>
                </label>
              </div>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-slate-800 rounded-xl border border-theme-surface-border w-full max-w-lg">
            ...
            <form onSubmit={handleAddStep} className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-theme-text-secondary mb-1">Step Name *</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block text-sm font-medium text-theme-text-secondary mb-1">Step Type</label>
                    ...
                    <button
                      ...
                      className={`p-3 rounded-lg border text-center transition-colors ${
                        stepFormData.step_type === type
                          ? 'border-red-500 bg-red-500/10 text-theme-text-primary'
                          : 'border-theme-input-border bg-slate-700/50 text-theme-text-muted hover:border-slate-500'
                      }`}
                    >
              ...
              {stepFormData.step_type === 'action' && (
                <div>
                  <label className="block text-sm font-medium text-theme-text-secondary mb-1">Action Type</label>
                  <select
              ...
              <div className="space-y-2">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    className="rounded border-theme-input-border bg-slate-700 text-red-700 dark:text-red-500 focus:ring-red-500"
                  />
                  <span className="text-sm text-theme-text-secondary">Required step</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
                    ...
                    className="rounded border-theme-input-border bg-slate-700 text-red-700 dark:text-red-500 focus:ring-red-500"
                  />
                  <span className="text-sm text-theme-text-secondary">Mark as first step (pipeline entry point)</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
                    ...
                    className="rounded border-theme-input-border bg-slate-700 text-red-700 dark:text-red-500 focus:ring-red-500"
                  />
                  <span className="text-sm text-theme-text-secondary">Mark as final step (approval / election)</span>
                </label>
              </div>
              <div className="flex justify-end gap-3 pt-4 border-t border-theme-surface-border">
                <button
