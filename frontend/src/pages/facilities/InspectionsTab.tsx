  ...
  const handleSave = async () => {
    ...
    try {
      const payload: InspectionCreate = {
        ...
        ...(formData.next_inspection_date ? { next_inspection_date: formData.next_inspection_date } : {}),
        ...(formData.inspector_name.trim() ? { inspector_name: formData.inspector_name.trim() } : {}),
        ...(formData.inspector_organization.trim() ? { inspector_organization: formData.inspector_organization.trim() } : {}),
        passed: formData.passed === 'true' ? true : formData.passed === 'false' ? false : null,
        ...(formData.findings.trim() ? { findings: formData.findings.trim() } : {}),
        ...(formData.corrective_actions.trim() ? { corrective_actions: formData.corrective_actions.trim() } : {}),
        ...(formData.corrective_action_deadline ? { corrective_action_deadline: formData.corrective_action_deadline } : {}),
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="relative flex-1 max-w-md">
          ...
          <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
            placeholder="Search inspections..." aria-label="Search inspections" className="form-input pl-10 placeholder-theme-text-muted pr-4 py-2.5"
          />
        </div>
        {filterFacilityId && (
