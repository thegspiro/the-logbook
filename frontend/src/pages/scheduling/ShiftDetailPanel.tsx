...
import {
  ...
  Loader2, AlertCircle, Phone, ClipboardList, ChevronDown, ChevronUp,
} from 'lucide-react';
...
export const ShiftDetailPanel: React.FC<ShiftDetailPanelProps> = ({
  ...
  const [assignForm, setAssignForm] = useState({ user_id: '', position: 'firefighter' });
  ...
  const handleSignup = async () => {
    setSigningUp(true);
    try {
      await schedulingService.signupForShift(shift.id, { position: signupPosition });
  ...
  const handleAssign = async () => {
    ...
    try {
      ...
      setAssignForm({ user_id: '', position: 'firefighter' });
      const data = await schedulingService.getShiftAssignments(shift.id);
      setAssignments(data as unknown as Assignment[]);
      onRefresh?.();
    } catch {
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex items-center gap-3 p-3 bg-theme-surface-hover/50 rounded-lg">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-theme-text-primary">{assignments.length} assigned</p>
              </div>
            </div>
            {shift.apparatus_id && (
              <div className="flex items-center gap-3 p-3 bg-theme-surface-hover/50 rounded-lg">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-theme-text-primary">{shift.apparatus_id}</p>
                </div>
              </div>
            )}
          ...
          <div>
            ...
            {showAssignForm && canManage && (
              <div className="mt-3 p-4 border border-theme-surface-border rounded-lg bg-theme-surface-hover/30 space-y-3">
          ...
          {/* Sign Up (for members not yet assigned) */}
          {!isPast && !isUserAssigned && !canManage && (
