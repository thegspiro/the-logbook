...
export const PlatoonSelector: React.FC<PlatoonSelectorProps> = ({ members, platoons, onPlatoonsChange }) => {
  ...
  return (
    <div className="space-y-3 border-t border-violet-500/10 pt-4">
      <div className="flex items-center justify-between">
        <h5 className="text-theme-text-secondary flex items-center gap-1.5 text-xs font-semibold tracking-wider uppercase">
          <Users className="h-3.5 w-3.5" /> Platoons
        </h5>
        <div className="flex items-center gap-1.5">
          <label className="text-theme-text-muted text-xs">Platoons</label>
          <select
            value={platoons.length || 3}
            onChange={(e) => onPlatoonsChange(PLATOON_NAMES.slice(0, parseInt(e.target.value, 10)))}
            className={selectCls}
          >
      ...
      <p className="text-theme-text-muted text-[11px]">
        Each platoon runs the same cycle offset by a few days so exactly one is on duty per day. Members are assigned to
        a platoon on their profile (Members &rarr; edit member); generated shifts are staffed from current platoon
        membership.
      </p>
      ...
      <div className="flex flex-wrap gap-2">
          ...
          return (
            <span
              ...
              className={`rounded-full border px-2 py-0.5 text-[11px] ${
                count > 0
                  ? 'text-theme-text-secondary border-violet-500/20 bg-violet-500/10'
        ...
        {unassignedCount > 0 && (
          <span className="border-theme-surface-border text-theme-text-muted rounded-full border px-2 py-0.5 text-[11px]">
      ...
      {platoons.every((p) => countForPlatoon(p) === 0) && (
        <p className="text-[11px] text-amber-600 dark:text-amber-400">
          No members are assigned to these platoons yet — shifts will be created but left unstaffed until members are
          given a platoon on their profile.
        </p>
      )}
    </div>
  );
