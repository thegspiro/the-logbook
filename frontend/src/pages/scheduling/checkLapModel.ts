...
/**
 * True when this container's contents can be taken on trust.
 *
 * A sealed bag with an intact tag is checked by reading the tag. Counting its
 * pockets anyway is not extra diligence — it is breaking the seal to confirm
 * what the seal already tells you, which destroys the evidence and costs the
 * crew ten minutes.
 */
...
export function isStopComplete(stop: LapStop, answers: AnswerMap): boolean {
  // A sealed bag is finished when its own line is answered — the tag. Its
  // pockets are not asked about at all while the seal holds.
  const items = contentsAreSealed(stop) ? ownAnswerableItems(stop) : answerableItems(stop);
