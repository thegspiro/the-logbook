 ...
 * Dashboard E2E Tests
 *
 * Covers the station-board layout: greeting, the thirty-day list that merges
...
test.describe('Dashboard', () => {
  ...
  test.describe('Next 30 Days', () => {
    test.beforeEach(async ({ page }) => {
      await gotoDashboard(page);
    });

    test('should display the Next 30 Days section heading', async ({ page }) => {
      const heading = page.getByRole('heading', { name: /next 30 days/i }).first();
    ...
    test('should render shift times rather than N/A', async ({ page }) => {
      const timeline = page.getByRole('region', { name: /next 30 days/i });
      await expect(timeline).toBeVisible({ timeout: 10000 });
      await expect(timeline.getByText('N/A')).toHaveCount(0);
    });

    test('should have an "All Shifts" control for the scheduling page', async ({ page }) => {
      const allShifts = page.getByRole('button', { name: /all shifts/i });
      await expect(allShifts.first()).toBeVisible({ timeout: 10000 });
      await expect(allShifts.first()).toHaveCSS('min-height', '44px');
    });

    // The control's whole job is the destination, and asserting only that it
    // is visible let a wrong one through unnoticed. The month view matters as
    // much as the route: the phone grid draws a month whatever the view says,
    // while the week view fetches only seven days to fill it.
    test('should open the shift calendar on the month view', async ({ page }) => {
      await page
        .getByRole('button', { name: /all shifts/i })
        .first()
        .click();

      await expect(page).toHaveURL(/\/scheduling\?.*view=month/, { timeout: 10000 });
    });
  });
  ...
  test.describe('Responsive Layout', () => {
    ...
    test('should stack the rail below the main column on narrow screens', async ({ page }) => {
      ...
      const timeline = page.getByRole('heading', { name: /next 30 days/i }).first();
      ...
      // Single-column layout: the rail sits below the thirty-day list.
