...
describe('authStore', () => {
  ...
  describe('logout', () => {
    ...
    it('clears temporary access token on logout', async () => {
      localStorage.setItem('has_session', '1');
      useAuthStore.setState({ user: fakeUser, isAuthenticated: true });
  ...
  describe('offline data at the account boundary', () => {
    ...
    describe('OAuth callback sign-in on a shared device', () => {
      // OAuthCallbackPage authenticates the member itself (the backend
      // callback already set the cookies) and never goes through login() —
      // it calls loadUser() directly, the same call a plain page reload
      // makes. Without marking that call a fresh sign-in first, a device
      // whose owner was never recorded reads exactly like the reload case
      // above and keeps whatever the previous member left behind.

      it('a bare loadUser() with no recorded owner does not purge — the reload case this could be mistaken for', async () => {
        localStorage.setItem('has_session', '1');
        mockGetCurrentUser.mockResolvedValue(fakeUser);
      ...
      it('markFreshSignIn() before loadUser() purges even with no recorded owner', async () => {
        // Mirrors OAuthCallbackPage.finish(): has_session is set by the
        // redirect handler, then the sign-in is marked fresh, then loadUser
        // resolves the member — all before any device_member_id exists.
        localStorage.setItem('has_session', '1');
        mockGetCurrentUser.mockResolvedValue(fakeUser);

        getState().markFreshSignIn();
        await act(async () => {
          await getState().loadUser();
        });
      ...
      it('markFreshSignIn() before loadUser() purges a different recorded owner too', async () => {
        localStorage.setItem('has_session', '1');
        localStorage.setItem('device_member_id', 'someone-else');
        mockGetCurrentUser.mockResolvedValue(fakeUser);
      ...
      it('markFreshSignIn() before loadUser() does not purge the same member’s own device', async () => {
        localStorage.setItem('has_session', '1');
        localStorage.setItem('device_member_id', 'u1');
        mockGetCurrentUser.mockResolvedValue(fakeUser);
